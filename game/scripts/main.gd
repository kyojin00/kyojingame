...
const LANDMARKS := [
	...
	{"id": "falls", "name": "큰폭포", "kind": "landmark_falls",
		...
		# **자리는 벼랑 줄에 맞춘다.** 그림은 밑변이 기준이고 높이가 6.5칸이라,
		# 마루가 벼랑 줄(x268 에서 y34)에 오려면 밑변이 그보다 여섯 칸
		# 아래여야 한다. 한 칸만 어긋나도 물이 벼랑 위나 밑에서 시작한다
		"tile": Vector2i(268, 28 + NORTH_PAD),
		# 물기둥이 지나는 칸을 막는다 — 안 막으면 폭포 속으로 걸어 들어가진다
		"block": Rect2i(-3, -6, 6, 7), "clear": 8,
