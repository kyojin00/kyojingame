...
const LANDMARKS := [
	...
	{"id": "falls", "name": "큰폭포", "kind": "landmark_falls",
		# 그림은 **벼랑 마루에서 못까지**만 맡는다 (화면 5.5칸 x 6.5칸).
		# 좌우로는 세계의 벼랑 타일이, 위아래로는 세계의 물 타일이 이어진다
		# **자리는 벼랑 줄에 맞춘다.** 그림은 밑변이 기준이고 높이가 4.5칸,
		# 마루는 그 위에서 0.25칸이다. 마루가 벼랑 줄(x268 에서 y34)에
		# 오려면 밑변이 y37 이어야 한다 — 한 칸만 어긋나도 물이 벼랑
		# 위에서 시작하거나 벼랑 밑 허공에서 시작한다
		"tile": Vector2i(268, 25 + NORTH_PAD),
		# 물기둥이 지나는 칸을 막는다 — 안 막으면 폭포 속으로 걸어 들어가진다
		"block": Rect2i(-3, -3, 6, 4), "clear": 9,
		# 벼랑 **위**의 못(켜 2)과 **밑**의 못(켜 1). 위 못은 마루까지 닿아
		# 있어서 거기서 물이 넘어간다
		"lakes": [[267, 17 + NORTH_PAD, 7.0, 5.0],
			[268, 29 + NORTH_PAD, 9.0, 5.0]],
		# 마루 바로 위 세 줄과 밑동 바로 밑 세 줄 — 여기는 흔들림 없이 물이다.
		# 못만 파 두었더니 가장자리 흔들림 때문에 한두 줄이 잔디로 남아,
		# 물이 벼랑에서 **뚝 떨어져 나온** 것처럼 끊겨 보였다
		# (마루가 y34, 밑동이 y37.1 이므로 그 바로 위 세 줄과 바로 밑 세 줄)
		"spill": [Rect2i(264, 19 + NORTH_PAD, 9, 3),
			Rect2i(264, 25 + NORTH_PAD, 9, 3)],
		"river": [266, 32 + NORTH_PAD, 246, 74 + NORTH_PAD, 2.0],
		# 폭포는 **진짜 벼랑**에서 떨어진다. 그림 왼쪽·오른쪽으로 그 벼랑이
		# 이어져 나가고, 서쪽 끝에 오르막이 있어 폭포 위로 올라갈 수 있다
		"terrain": {"blobs": [[260, 12 + NORTH_PAD, 16.0, 11.0, 2, 91]],
			"ramps": [[248, 23 + NORTH_PAD]]}},
