...
func _dc_fill(x: int, y: int, ci: int, i: int, above: PackedByteArray,
	...
	if (kn & 56) != lvb or (ks & 56) != lvb or (kw & 56) != lvb \
		...
		# 그런데 그 칸에도 벽을 그리고 있었다 — 옆면 붓이 경계의 기울기만
		# 보므로, 북쪽 가장자리가 조금만 비뚤어도 회색 띠가 한 장씩 섰다.
		# 대지의 **뒷면마다 회색 실이 토막토막** 흩어져 보인 것이 이것이다.
		#
		# 그릴 값이 있는 것은 북·서·동이 높을 때뿐이다 (1 | 4 | 8 = 13).
		# 남쪽만 높은 칸은 그 땅의 마루선(brink)이 대신 말해 준다.
		if gc != K_WATER:
			if (up & 13) != 0:
