...
var appearance := {"hair": 0, "shirt": 0, "pants": 0, "shoes": 0,
	"skin": 0, "hair_col": 0}
# 도트 한 벌이 곧 한 사람이라, 고르는 것은 「머리 모양」이 아니라
# **누구로 시작하느냐**다. 넷이던 것이 둘로 줄었다 — 옛 네 벌은 손으로
# 그린 것이고 지금 두 벌은 그림체가 아예 달라, 섞어 두면 같은 게임 안에
# 두 화풍이 나온다. 한 벌을 더 그려 넣으면 여기에 줄만 더하면 된다.
const HAIR_PREFIX := ["new_boy", "player_f"]
const HAIR_NAMES := ["남자아이", "여자아이"]
# ---- 색 표 --------------------------------------------------------------
#
# 예전 방식(팩 팔레트)은 **색을 하나씩 맞바꾸는** 것이었다. 한 재질이 다섯
# 단(기본·그늘·밝은 면·깊은 그늘·아주 밝은 면)뿐이라 표에 다섯 색을 적어
# 두고 그대로 갈아 끼우면 됐다.
#
# 새 도트는 손으로 그린 것이 아니라 한 재질 안에 색이 **수십 가지**고, 반대로
# 서로 다른 재질이 **같은 색**을 쓴다 (검정은 윤곽이면서 동시에 남자 바지다).
# 맞바꾸기로는 둘 다 못 다룬다 — 다 적을 수도 없고, 적으면 윤곽까지 물든다.
#
# 그래서 두 가지로 바꿨다.
#   어디가 무엇인가 : 도트마다 **재질판**을 한 장씩 굽는다 (assets/sprites/mat/,
#                    ref/pack/make_mat.py). 한 칸에 0~5 가 적혀 있다.
#   어떻게 물들이나 : 표에는 재질마다 **기본색 한 칸**만 적는다. 그려진 기본색이
#                    고른 색으로 옮겨 가는 만큼, 같은 재질의 다른 색들도 명도·
#                    채도의 **비율**을 지킨 채 따라 옮긴다 (_shift_color).
#
# 0번은 어디서나 「그린 그대로」다. 사람마다 그려진 색이 달라 표에 적을 수가
# 없어서 값은 비워 두고(검정), 고르면 아무것도 안 건드린다. 화면의 네모는
# PC_BASE 에서 지금 고른 사람의 색을 가져다 칠한다 (title.gd).
const SHIRT_NAMES := ["그린 그대로", "잿빛 청록", "흙빛 분홍", "이끼", "모래", "자주"]
const PANTS_NAMES := ["그린 그대로", "밤색", "남색", "올리브", "팥죽", "잿빛"]
const SHOES_NAMES := ["그린 그대로", "검정", "밤색", "남색"]
const SKIN_NAMES := ["그린 그대로", "흰 살결", "볕에 그은", "구릿빛", "갈색", "짙은 갈색"]
const HAIR_COL_NAMES := ["그린 그대로", "검정", "아마빛", "주황빛", "잿빛", "연분홍",
	"청회색", "짙은 갈색"]

const APPEAR_SHIRT := [
	[0, 0, 0], [96, 140, 138], [196, 136, 140], [118, 142, 104],
	[206, 182, 140], [140, 110, 150],
]
const APPEAR_PANTS := [
	[0, 0, 0], [112, 86, 66], [66, 78, 108], [104, 102, 72],
	[116, 74, 80], [104, 104, 110],
]
const APPEAR_SHOES := [
	[0, 0, 0], [86, 84, 92], [126, 92, 68], [78, 92, 124],
]
const APPEAR_SKIN := [
	[0, 0, 0], [255, 238, 224], [236, 196, 158], [208, 160, 120],
	[168, 120, 86], [124, 86, 62],
]
const APPEAR_HAIR_COL := [
	[0, 0, 0], [62, 56, 60], [196, 164, 112], [172, 104, 68], [146, 142, 148],
	[198, 146, 166], [104, 128, 158], [96, 70, 56],
]

# 각 벌에 **그려져 있는** 기본색 — 물들이기의 기준점이다. 여기서 고른 색으로
# 얼마나 옮겨 가는지를 재고, 같은 재질의 나머지 색을 그 비율로 따라 옮긴다.
# (ref/pack/make_mat.py 의 재질 가르기와 같은 색을 쓴다)
const PC_BASE := {
	"new_boy": {
		"skin": [251, 217, 186], "hair": [170, 98, 60], "shirt": [118, 134, 151],
		"pants": [31, 28, 42], "shoes": [247, 242, 228],
	},
	"player_f": {
		"skin": [251, 204, 177], "hair": [70, 34, 77], "shirt": [247, 234, 213],
		"pants": [92, 155, 145], "shoes": [243, 213, 216],
	},
}

# 재질판에 적힌 번호 (make_mat.py 가 40씩 띄워 굽는다 — 0·40·80·…)
const MAT_STEP := 40
const MAT_SKIN := 1
const MAT_HAIR := 2
const MAT_TOP := 3
...
# 그려진 기준색 src 가 고른 색 dst 로 옮겨 갈 때, 같은 재질의 다른 색 c 가
# 어디로 가야 하는지.
#
# 색상(H)은 **반만** 따라간다. 한 재질 안의 색상 차이는 볕과 그늘이 만든
# 것이라(밝은 쪽이 노랗고 어두운 쪽이 붉다) 그대로 옮기면 금발 끝이 초록으로
# 튄다. 최대 0.06까지만 벌린다.
#
# 채도·명도는 기준색을 꼭짓점으로 하는 꺾은선으로 옮긴다. 기준색보다 어두운
# 쪽은 비율 그대로, 밝은 쪽은 기울기를 1 아래로 눌러 둔다 — 안 누르면 검은
# 머리로 바꿨을 때 윤기가 회색 줄무늬로 남는다.
static func _knee(x: float, b: float, d: float) -> float:
	if b <= 0.0 or b >= 1.0:
		return d
	if x <= b:
		return d * (x / b)
	return minf(1.0, d + (x - b) * minf((1.0 - d) / (1.0 - b), maxf(1.0, d / b)))


static func shift_color(c: Color, src: Color, dst: Color) -> Color:
	# 기준색 자신은 고른 색에 **정확히** 떨어져야 한다 (HSV를 오갔다 오면
	# 1씩 어긋나고, 그러면 「골랐는데 그 색이 한 칸도 없다」가 된다)
	if c.r8 == src.r8 and c.g8 == src.g8 and c.b8 == src.b8:
		return Color(dst.r, dst.g, dst.b, c.a)
	var d := fposmod(c.h - src.h + 0.5, 1.0) - 0.5
	d = clampf(d * 0.5, -0.06, 0.06)
	var out := Color.from_hsv(fposmod(dst.h + d, 1.0),
		_knee(c.s, src.s, dst.s), _knee(c.v, src.v, dst.v))
	out.a = c.a
	return out


# 고른 색으로 도트 한 장을 물들인다. mat 은 같은 크기의 재질판이다.
func recolor_player_image(img: Image, mat: Image, pre: String, ap: Dictionary) -> void:
	var base: Dictionary = PC_BASE.get(pre, {})
	if base.is_empty():
		return
	var job := {}          # 재질 번호 -> [그려진 색, 고른 색]
	for t in [[MAT_HAIR, APPEAR_HAIR_COL, ap.get("hair_col", 0), "hair"],
			[MAT_SKIN, APPEAR_SKIN, ap.get("skin", 0), "skin"],
			[MAT_TOP, APPEAR_SHIRT, ap.get("shirt", 0), "shirt"],
			[MAT_BOT, APPEAR_PANTS, ap.get("pants", 0), "pants"],
			[MAT_SHOE, APPEAR_SHOES, ap.get("shoes", 0), "shoes"]]:
		var tbl: Array = t[1]
		var sel: int = clampi(int(t[2]), 0, tbl.size() - 1)
		if sel == 0:
			continue          # 「그린 그대로」
		var b: Array = base[t[3]]
		var q: Array = tbl[sel]
		job[t[0]] = [Color8(b[0], b[1], b[2]), Color8(q[0], q[1], q[2])]
	if job.is_empty():
		return
	# 같은 색은 한 번만 계산한다 (한 장에 색이 백 가지라도 칸은 천오백 개다)
	var seen := {}
	for y in img.get_height():
		for x in img.get_width():
			...
			if c.a == 0.0:
				continue
			var m := int(round(mat.get_pixel(x, y).r8 / float(MAT_STEP)))
			if not job.has(m):
				continue
			var key: int = (m << 32) | int(c.to_rgba32() & 0xFFFFFFFF)
			if not seen.has(key):
				seen[key] = shift_color(c, job[m][0], job[m][1])
			img.set_pixel(x, y, seen[key])
...
# 벌리는 「딛기」가 두 번 있어야 한다 — 통과는 위상 0도와 180도다.
# 다섯 칸은 눈금 간격이 72도라 180도에 눈금이 없어, 두 발짝 중 하나만
# 몸이 통통 뜨는 절뚝걸음이었다. **네 칸**이면 눈금이 90도라 딛기와 통과가
# 정확히 번갈아 놓인다. 지금 도트가 딛기·통과·딛기·통과 네 장이다.
#
# 한 바퀴는 예전과 같은 0.5초로 둔다 — 네 칸이니 초당 여덟 칸.
# (이동 속도 150px/s면 한 발짝에 37px 남짓, 다리 길이에 가까워 안 미끄러진다)
const WALK_FRAMES := 4
const WALK_FPS := 8.0
...
func build_save(grid_data: Array, player_pos: Vector2, objects_data: Array = [],
	...
	return {
		...
		"appear_v": 2,        # 머리 스타일 넷 -> 사람 둘. 없으면 옛 번호로 본다
