...
# ---- 색 표 (daily_rpg_pair_pack 팔레트 기준) ------------------------------
# 팩은 남자와 여자가 **서로 다른 색의 옷**을 입고 있다 (남자 푸른 맨투맨·회청
# 바지, 여자 크림 블라우스·청록 치마, 머리도 갈색과 보라). recolor 는 0행
# 하나만 표준으로 삼으므로, 옷·머리 표의 0행에 **두 벌을 나란히** 담는다.
#   슬롯 0~4 = 남자 벌 다섯 단,  슬롯 5~9 = 여자 벌 다섯 단
# 다른 행(선택지)은 두 묶음에 같은 색을 넣는다 — 어느 쪽 도트를 쓰든 고른
# 색으로 갈아입혀지고, 0행을 고르면 둘 다 그려진 그대로 남는다.
# 신발·살결은 남녀가 같은 색을 쓰므로 한 묶음이면 된다.
#
# 팩이 재질끼리 색을 공유하던 자리는 도트 쪽에서 떼어냈다 (separate_pack.py):
#   운동화 갑피·양말·블라우스 밝은면이 살결 밝은면(255,244,223)과 같았고,
#   신발 윤곽이 남자 바지 기본색(58,57,74)과 같았다. 겹친 채로 두면 바지를
#   갈아입을 때 신발 윤곽까지 같이 바뀐다.
const SHIRT_NAMES := ["팩 기본", "잿빛 청록", "흙빛 분홍", "이끼", "모래", "자주"]
const PANTS_NAMES := ["팩 기본", "밤색", "남색", "올리브", "팥죽", "잿빛"]
const SHOES_NAMES := ["흰 운동화", "검정", "밤색", "남색"]
const SKIN_NAMES := ["살구", "흰 살결", "볕에 그은", "구릿빛", "갈색", "짙은 갈색"]
const HAIR_COL_NAMES := ["팩 기본", "검정", "아마빛", "주황빛", "잿빛", "연분홍", "청회색", "짙은 갈색"]

# 윗도리 — 앞 다섯 칸이 남자 벌, 뒤 다섯 칸이 여자 벌.
# 0행만 둘이 다르고, 나머지 행은 두 묶음에 같은 색을 넣어 어느 쪽
# 도트를 쓰든 고른 색으로 갈아입혀지게 한다.
# 각 묶음 순서: [기본, 그늘, 밝은 면, 깊은 그늘, 아주 밝은 면]
const APPEAR_SHIRT := [
	[[89, 133, 159], [54, 94, 122], [128, 167, 186], [36, 70, 88], [181, 205, 209], [243, 236, 215], [222, 222, 206], [254, 247, 228], [172, 166, 154], [255, 252, 244]],  # 팩 기본 — 남자 푸른 맨투맨 · 여자 크림 블라우스
	[[96, 140, 138], [65, 108, 106], [127, 164, 162], [45, 77, 76], [166, 183, 183], [96, 140, 138], [65, 108, 106], [127, 164, 162], [45, 77, 76], [166, 183, 183]],  # 잿빛 청록
	[[196, 136, 140], [151, 92, 96], [229, 179, 183], [108, 64, 67], [255, 232, 233], [196, 136, 140], [151, 92, 96], [229, 179, 183], [108, 64, 67], [255, 232, 233]],  # 흙빛 분홍
	[[118, 142, 104], [86, 109, 72], [146, 166, 135], [60, 78, 50], [177, 186, 171], [118, 142, 104], [86, 109, 72], [146, 166, 135], [60, 78, 50], [177, 186, 171]],  # 이끼
	[[206, 182, 140], [159, 135, 94], [241, 221, 186], [113, 96, 65], [255, 246, 230], [206, 182, 140], [159, 135, 94], [241, 221, 186], [113, 96, 65], [255, 246, 230]],  # 모래
	[[140, 110, 150], [106, 76, 116], [167, 142, 176], [75, 53, 82], [193, 181, 196], [140, 110, 150], [106, 76, 116], [167, 142, 176], [75, 53, 82], [193, 181, 196]],  # 자주
]

# 아랫도리 — 남자 바지 다섯 칸 + 여자 치마 다섯 칸
const APPEAR_PANTS := [
	[[58, 57, 74], [44, 43, 58], [83, 86, 103], [31, 30, 42], [115, 125, 139], [70, 139, 140], [40, 102, 108], [116, 170, 170], [28, 74, 79], [168, 205, 203]],  # 팩 기본 — 남자 회청 바지 · 여자 청록 치마
	[[112, 86, 66], [86, 61, 41], [131, 109, 93], [62, 42, 28], [147, 137, 129], [112, 86, 66], [86, 61, 41], [131, 109, 93], [62, 42, 28], [147, 137, 129]],  # 밤색
	[[66, 78, 108], [42, 54, 83], [91, 101, 126], [28, 37, 59], [125, 130, 141], [66, 78, 108], [42, 54, 83], [91, 101, 126], [28, 37, 59], [125, 130, 141]],  # 남색
	[[104, 102, 72], [80, 78, 49], [122, 120, 95], [57, 56, 34], [136, 135, 124], [104, 102, 72], [80, 78, 49], [122, 120, 95], [57, 56, 34], [136, 135, 124]],  # 올리브
	[[116, 74, 80], [89, 48, 54], [136, 101, 106], [64, 33, 37], [152, 135, 138], [116, 74, 80], [89, 48, 54], [136, 101, 106], [64, 33, 37], [152, 135, 138]],  # 팥죽
	[[104, 104, 110], [79, 79, 85], [124, 124, 129], [56, 56, 61], [142, 142, 144], [104, 104, 110], [79, 79, 85], [124, 124, 129], [56, 56, 61], [142, 142, 144]],  # 잿빛
]

# 신발 — 남녀가 같은 색을 쓰므로 한 묶음.
# [기본, 그늘, 밝은 면, 윤곽, 양말]
const APPEAR_SHOES := [
	[[217, 223, 212], [200, 199, 188], [244, 247, 240], [64, 62, 78], [236, 240, 230]],  # 흰 운동화
	[[86, 84, 92], [64, 62, 70], [112, 110, 120], [38, 36, 44], [150, 150, 156]],  # 검정
	[[126, 92, 68], [96, 68, 50], [154, 118, 90], [60, 42, 30], [196, 172, 148]],  # 밤색
	[[78, 92, 124], [56, 68, 96], [104, 120, 152], [34, 42, 62], [156, 168, 192]],  # 남색
]

# 피부 — 남녀 공통 일곱 칸.
# [기본, 밝은 면, 그늘, 깊은 그늘, 가장 깊은 그늘, 볼, 입]
const APPEAR_SKIN := [
	[[255, 227, 201], [255, 244, 223], [245, 201, 177], [191, 141, 131], [104, 71, 73], [234, 162, 162], [199, 97, 133]],  # 살구
	[[255, 238, 224], [255, 250, 242], [248, 216, 200], [204, 160, 152], [118, 84, 86], [240, 178, 178], [206, 112, 146]],  # 흰 살결
	[[236, 196, 158], [248, 220, 190], [216, 166, 128], [168, 118, 100], [92, 62, 60], [218, 144, 132], [186, 88, 116]],  # 볕에 그은
	[[208, 160, 120], [226, 188, 154], [184, 130, 92], [142, 94, 74], [78, 50, 46], [196, 120, 104], [166, 74, 98]],  # 구릿빛
	[[168, 120, 86], [190, 148, 114], [142, 94, 64], [110, 70, 54], [60, 38, 34], [158, 92, 76], [136, 58, 78]],  # 갈색
	[[124, 86, 62], [146, 110, 84], [100, 66, 46], [78, 50, 38], [44, 28, 24], [118, 66, 54], [104, 44, 60]],  # 짙은 갈색
]

# 머리카락 — 남자 벌 다섯 칸 + 여자 벌 다섯 칸
const APPEAR_HAIR_COL := [
	[[146, 112, 98], [112, 81, 74], [179, 141, 116], [75, 57, 57], [205, 178, 156], [107, 96, 131], [80, 70, 98], [128, 117, 150], [55, 49, 70], [150, 139, 172]],  # 팩 기본 — 남자 갈색 · 여자 보라
	[[62, 56, 60], [48, 43, 46], [76, 68, 74], [32, 29, 31], [87, 81, 85], [62, 56, 60], [48, 43, 46], [76, 68, 74], [32, 29, 31], [87, 81, 85]],  # 검정
	[[196, 164, 112], [151, 126, 84], [241, 199, 131], [100, 88, 69], [255, 225, 175], [196, 164, 112], [151, 126, 84], [241, 199, 131], [100, 88, 69], [255, 225, 175]],  # 아마빛
	[[172, 104, 68], [132, 79, 50], [212, 122, 75], [88, 62, 49], [241, 171, 135], [172, 104, 68], [132, 79, 50], [212, 122, 75], [88, 62, 49], [241, 171, 135]],  # 주황빛
	[[146, 142, 148], [112, 109, 114], [179, 174, 182], [75, 73, 75], [205, 201, 207], [146, 142, 148], [112, 109, 114], [179, 174, 182], [75, 73, 75], [205, 201, 207]],  # 잿빛
	[[198, 146, 166], [152, 111, 127], [244, 175, 201], [101, 82, 89], [255, 206, 225], [198, 146, 166], [152, 111, 127], [244, 175, 201], [101, 82, 89], [255, 206, 225]],  # 연분홍
	[[104, 128, 158], [79, 98, 122], [123, 155, 194], [60, 69, 81], [166, 191, 221], [104, 128, 158], [79, 98, 122], [123, 155, 194], [60, 69, 81], [166, 191, 221]],  # 청회색
	[[96, 70, 56], [74, 53, 42], [118, 84, 65], [49, 39, 34], [134, 108, 94], [96, 70, 56], [74, 53, 42], [118, 84, 65], [49, 39, 34], [134, 108, 94]],  # 짙은 갈색
]
...
const NPC_KIND := {
	...
	"officer_park": "core",
	# 갈뫼읍 사람(S4c) — 도심의 아는 얼굴. 교진 주민도 노트의 「필수」도 아니다
	"mayor_kang": "town", "chief_ha": "town", "judge_suh": "town", "pros_min": "town",
	"doctor_oh": "town", "manager_baek": "town", "innkeeper_ok": "town", "cook_jang": "town",
	"fence_gu": "town",
}
...
const NPCS := {
	...
	# ---- 갈뫼읍 손글 아홉(S4c) — 도심의 아는 얼굴. 관청마다 우두머리 하나, 여관 뒷방에 장물아비 ----
	"mayor_kang": {"name": "강 군수", "birthday": [SPRING, 3], "gender": "m", "romance": false,
	"lines": [
		"군수는 도장 찍는 사람일세. 도장 하나에 마을 하나가 달렸지.",
		"교진 이장하고는 젊어서부터 아는 사이야. 그 양반이 나보다 낫네.",
		"예산은 물 같아서 새는 데부터 막아야 하네.",
		"읍이 커지면 좋겠나. 나는 이만하면 됐네.",
		"자네 얼굴은 기억해 두지. 군수는 얼굴로 일하네.",
	],
	"season": {
		SPRING: ["봄엔 도로부터 손보네. 길이 마을을 먹여 살리지."],
		SUMMER: ["장마 전에 둑을 봐야 하는데 사람이 없네."],
		FALL: ["추수철엔 세금 얘기 꺼내기가 미안하지."],
		WINTER: ["겨울엔 군청이 제일 따뜻하네. 난로 옆에 앉게."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오면 민원이 준다네. 다들 집에 있으니."],
		WEATHER_STORM: ["폭풍이면 나부터 나가 봐야 하네. 군수라서."],
		WEATHER_FOG: ["안개 낀 날은 서류가 잘 안 읽혀."],
	},
	"morning": ["아침 결재부터 하세. 도장이 식기 전에."],
	"night": ["이 시간에 군청은 닫았네. 내일 오게."],
	"aff30": ["자네 마을 얘기를 이장한테 들었네. 부지런하더군.", "군청은 무서운 데가 아닐세. 자주 오게."],
	"aff70": ["언젠가 자네가 이 자리에 앉을지도 모르지. 그러길 바라네.", "내 도장을 자네한테 맡길 날이 올까."],
	"loves": ["dish_pie", "gem"],
	"likes": ["dish_bread", "egg"],
	"hates": ["forage_trash", "sludge"],
	"secret50": "젊어서 교진에 살았네. 자네 할아버지 밭에서 감자를 캤지.\n군수가 된 건 그 밭이 그리워서일세.",
	"secret100": "교진 도로 포장은 내가 도에 세 번 청원했네.\n자네가 낸 세금이 길이 됐다면 그건 자네 덕이지 내 덕이 아니야.",
	},
	"chief_ha": {"name": "하 서장", "birthday": [SUMMER, 9], "gender": "m", "romance": false,
	"lines": [
		"경찰서는 밤에 바쁘네. 낮에 온 자넨 손님이지.",
		"박 순경은 내가 교진으로 보냈네. 좋은 사람이야.",
		"읍엔 순경이 셋일세. 셋이 도는 밤은 안전하네.",
		"죄는 사람이 짓고 벌은 법이 주네. 나는 그 사이일세.",
		"자네 얼굴, 수배지에서 본 적 없지. 그럼 됐네.",
	],
	"season": {
		SPRING: ["봄엔 낯선 얼굴이 늘어. 다 손님이면 좋겠는데."],
		SUMMER: ["여름밤은 길고 사고도 길지."],
		FALL: ["추수철엔 창고 도둑이 는다네. 문단속 이르게."],
		WINTER: ["눈 오는 밤은 발자국이 남아 좋지."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오는 밤엔 순찰이 젖네. 그래도 도네."],
		WEATHER_STORM: ["폭풍이면 순경 셋이 다 나가네. 나도."],
		WEATHER_FOG: ["안개는 도둑 편일세."],
	},
	"morning": ["밤새 보고서를 읽었네. 조용했어."],
	"night": ["이 시간엔 순경들이 돌고 있네. 자넨 들어가게."],
	"aff30": ["자네 마을 순경한테 자네 얘길 들었네. 밤길을 잘 돈다더군.", "경찰서 문은 늘 열려 있네."],
	"aff70": ["제복이 필요하면 말하게. 자네한테는 맞을 걸세.", "자네 같은 사람이 순경이면 내가 잠을 자지."],
	"loves": ["dish_soup", "lamp"],
	"likes": ["dish_bread", "fish_carp"],
	"hates": ["sludge", "forage_trash"],
	"secret50": "박 순경이 읍에서 놓친 사람, 내가 놓친 걸세.\n그 일을 그 사람이 떠안았지. 미안한 채로 산다네.",
	"secret100": "자네 할아버지가 여기 순경이었을 때 나는 신참이었네.\n그 양반이 가르친 걸 지금도 순경들한테 가르치네.",
	},
	"judge_suh": {"name": "서 부장", "birthday": [FALL, 12], "gender": "m", "romance": false,
	"lines": [
		"법원은 조용해야 하네. 소리는 밖에서 내게.",
		"윤 판사가 교진을 도네. 나는 여기 앉아 있고.",
		"판결은 무겁게 내려야 하네. 가벼우면 아무도 안 듣네.",
		"책을 열두 권은 읽어야 법을 읽네.",
		"피고석에 서 본 사람이 판사석을 안다네. 자넨 어느 쪽인가.",
	],
	"season": {
		SPRING: ["봄엔 사건이 준다네. 다들 밭에 있으니."],
		SUMMER: ["여름엔 다툼이 늘어. 더워서 그런가."],
		FALL: ["가을 재판은 창고 얘기가 많네."],
		WINTER: ["겨울 법정은 춥네. 그래도 문은 여네."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오는 날 재판은 방청석이 비지."],
		WEATHER_STORM: ["폭풍이면 재판을 미루네. 판사도 사람일세."],
		WEATHER_FOG: ["안개 낀 날 판결은 더 신중해지네."],
	},
	"morning": ["아침엔 어제 판결문을 다시 읽네. 고칠 게 없나."],
	"night": ["법원은 닫았네. 밤에 법은 쉬네."],
	"aff30": ["자네가 책을 읽는다고 서하한테 들었네. 계속 읽게.", "법정 방청은 언제든 오게."],
	"aff70": ["자네가 판사석에 앉으면 나는 방청석에서 보겠네.", "판사는 외로운 자리일세. 자넨 견딜 걸세."],
	"loves": ["dish_moon_tea", "old_book"],
	"likes": ["dish_bread", "dish_pickle"],
	"hates": ["forage_trash", "ghost_essence"],
	"secret50": "첫 판결로 사람 하나를 잘못 가뒀네.\n그 뒤로 판결문을 세 번 읽네. 자네도 그리하게.",
	"secret100": "윤 판사는 내 제자일세. 교진에 보낸 건 자네 같은 사람을 만나라고였네.\n판사석은 내가 아니라 그 사람이 넘길 걸세.",
	},
	"pros_min": {"name": "민 부장", "birthday": [WINTER, 21], "gender": "f", "romance": false,
	"lines": [
		"검찰청은 서류로 말하네. 서류 없이는 말도 없네.",
		"한 검사가 교진을 맡지. 나는 읍 사건을 보네.",
		"증거가 문턱을 넘어야 기소하네. 감으로는 안 하네.",
		"덮으라는 사람이 늘 있네. 안 덮는 게 일일세.",
		"자네 이름은 서류에 없네. 그대로 있게.",
	],
	"season": {
		SPRING: ["봄엔 새 사건이 적어 서류를 정리하네."],
		SUMMER: ["여름 서류는 땀에 젖네."],
		FALL: ["가을엔 창고 사건 서류가 쌓이지."],
		WINTER: ["겨울엔 난로 옆에서 서류를 읽네."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오는 날은 서류가 눅눅하네."],
		WEATHER_STORM: ["폭풍이면 서류부터 챙기네."],
		WEATHER_FOG: ["안개 낀 날은 증인이 적네."],
	},
	"morning": ["아침 서류 한 무더기. 오늘도 읽네."],
	"night": ["검찰청은 닫았네. 서류도 잠을 자네."],
	"aff30": ["자네 마을 순경 보고서를 읽었네. 자네 이름이 좋게 나오더군.", "서류 보는 법을 가르쳐 줄 수도 있네."],
	"aff70": ["자네가 검사 자리에 앉으면 서류를 나눠 들겠네.", "덮으라는 말을 자네도 듣게 될 걸세. 그때 나를 떠올리게."],
	"loves": ["dish_salad", "cloth"],
	"likes": ["dish_bread", "forage_berry"],
	"hates": ["sludge", "forage_trash"],
	"secret50": "덮으라는 사건을 한 번 덮었네. 그 뒤로 한 번도 안 덮었지.\n한 번이 평생을 가네.",
	"secret100": "자네 할아버지 사건 서류를 내가 봤네. 무죄였네.\n그 서류는 지금도 내 서랍에 있네. 언젠가 보여 주지.",
	},
	"doctor_oh": {"name": "오 소장", "birthday": [SPRING, 27], "gender": "f", "romance": false,
	"lines": [
		"보건소는 아픈 사람이 오는 데지 아픈 사람을 만드는 데가 아닐세.",
		"교진에 진료소가 서면 간호사를 보내 주겠네.",
		"약은 밭에서 나네. 약초를 알면 반은 의사일세.",
		"기절한 사람은 하루 쉬어야 하네. 자네도.",
		"손 씻고 들어오게. 그게 첫 진료일세.",
	],
	"season": {
		SPRING: ["봄엔 꽃가루로 오는 사람이 많네."],
		SUMMER: ["여름엔 더위 먹은 사람이 오지. 물을 마시게."],
		FALL: ["가을엔 다들 튼튼하네. 보건소가 한가해."],
		WINTER: ["겨울 감기가 제일 무섭네. 따뜻이 입게."],
	},
	"weather": {
		WEATHER_RAIN: ["비 맞고 다니면 여기 오게 되네."],
		WEATHER_STORM: ["폭풍이면 다친 사람이 오지. 조심하게."],
		WEATHER_FOG: ["안개는 몸에 안 좋네."],
	},
	"morning": ["아침 진료는 아홉 시부터일세."],
	"night": ["밤엔 급한 사람만 받네. 자넨 급한가."],
	"aff30": ["자네 약초 캐는 손이 좋다고 들었네.", "기절하면 여기로 오게. 흉이 아닐세."],
	"aff70": ["자네가 흰옷을 입으면 어울리겠네.", "간호사 자리가 비었네. 자네를 생각하고 있었네."],
	"loves": ["forage_herb", "dish_soup"],
	"likes": ["forage_berry", "egg"],
	"hates": ["sludge", "forage_trash"],
	"secret50": "읍에 병원이 없어 사람을 하나 잃었네. 그때 이 보건소를 열었지.\n하나라도 덜 잃으려고.",
	"secret100": "자네 할아버지가 마지막에 여기 왔었네. 편안히 가셨네.\n그 얘기를 자네한테 하려고 기다렸네.",
	},
	"manager_baek": {"name": "백 지점장", "birthday": [SUMMER, 30], "gender": "m", "romance": false,
	"lines": [
		"신협은 남의 돈을 맡는 데지 내 돈을 쓰는 데가 아닐세.",
		"장부는 거짓말을 안 하네. 사람이 하지.",
		"교진 사람들은 저축을 안 하네. 자네가 첫 손님이 되게.",
		"이자는 시간의 값일세. 시간이 없으면 이자도 없네.",
		"쇠창살은 도둑 막으려는 게 아닐세. 나를 막으려는 걸세.",
	],
	"season": {
		SPRING: ["봄엔 씨앗값 빌리러 오는 사람이 많네."],
		SUMMER: ["여름엔 장부가 조용하네."],
		FALL: ["추수 뒤엔 다들 갚으러 오지. 좋은 철일세."],
		WINTER: ["겨울엔 이자만 도네."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오면 손님이 없어 장부를 다시 세네."],
		WEATHER_STORM: ["폭풍이면 금고부터 잠그네."],
		WEATHER_FOG: ["안개 낀 날은 숫자가 흐리네."],
	},
	"morning": ["아침엔 금고를 열고 어제 장부를 맞추네."],
	"night": ["신협은 닫았네. 돈도 잠을 자네."],
	"aff30": ["자네 좌판 장부를 봤네. 깔끔하더군.", "저축을 시작하게. 작게라도."],
	"aff70": ["자네한테는 창구를 맡겨도 되겠네.", "장부를 읽는 눈이 자네한테 있네."],
	"loves": ["gem", "dish_pie"],
	"likes": ["dish_bread", "forage_ring"],
	"hates": ["forage_trash", "sludge"],
	"secret50": "젊어서 장부를 한 줄 고쳤네. 아무도 몰랐지.\n그 한 줄이 아직 나를 재네.",
	"secret100": "자네 할아버지 통장이 여기 있네. 한 번도 안 찾았어.\n자네 이름으로 바꿔 두겠네.",
	},
	"innkeeper_ok": {"name": "옥이 주모", "birthday": [FALL, 5], "gender": "f", "romance": false,
	"lines": [
		"여관은 잠자는 데지 사는 데가 아니에요. 그래도 사는 사람이 있죠.",
		"뒷방 구씨는 내 손님이에요. 뭘 하는진 안 물어요.",
		"국 한 그릇 드릴까요. 길 온 사람 얼굴이네.",
		"교진 사람은 여기 잘 안 자요. 집이 가까우니까.",
		"밤 손님이 무섭진 않아요. 국자가 있으니까.",
	],
	"season": {
		SPRING: ["봄엔 장꾼이 많이 자요."],
		SUMMER: ["여름밤은 마루에서 자는 손님도 있어요."],
		FALL: ["가을엔 방이 다 차요. 미리 말해요."],
		WINTER: ["겨울엔 아랫목이 제일 비싸요."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오면 다들 여기서 자요. 방이 없어요."],
		WEATHER_STORM: ["폭풍엔 문을 안 잠가요. 누구든 들어오라고."],
		WEATHER_FOG: ["안개 낀 날 손님은 조용해요."],
	},
	"morning": ["아침 국은 여섯 시부터예요."],
	"night": ["방 남았어요. 하룻밤 어때요."],
	"aff30": ["교진 얘기 좀 해 줘요. 나도 거기 살았거든요.", "국이 식어요. 어서 먹어요."],
	"aff70": ["자기가 오면 아랫목을 비워 둬요.", "뒷방 구씨가 자길 찾더라. 조심해요."],
	"loves": ["dish_stew", "flour"],
	"likes": ["dish_bread", "egg"],
	"hates": ["sludge", "forage_trash"],
	"secret50": "교진에서 시집왔다가 돌아왔어요. 남편은 바다에 있고요.\n여관을 연 건 돌아올 사람을 기다리려고예요.",
	"secret100": "자기 할아버지가 여기 자고 간 적 있어요. 국을 두 그릇 먹었죠.\n손자 얘기를 하더라고요. 그게 자기였네.",
	},
	"cook_jang": {"name": "장씨", "birthday": [WINTER, 2], "gender": "m", "romance": false,
	"lines": [
		"식당은 맛으로 말해. 말은 짧게.",
		"교진 생선이 제일이야. 용식이한테 받아.",
		"국솥은 새벽에 걸어. 낮에 오면 늦어.",
		"돈 없으면 설거지해. 한 그릇은 줘.",
		"자네 요리 솜씨 들었어. 한번 만들어 봐.",
	],
	"season": {
		SPRING: ["봄나물 무침이 나와. 먹어 봐."],
		SUMMER: ["여름엔 냉국이야."],
		FALL: ["가을 생선이 기름져. 지금이 철이야."],
		WINTER: ["겨울엔 국밥이지."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오면 부침개야. 안 물어봐도 돼."],
		WEATHER_STORM: ["폭풍엔 문 닫아. 솥이 넘어져."],
		WEATHER_FOG: ["안개 낀 날은 국이 더 맛있어."],
	},
	"morning": ["아침 국 나왔어. 앉아."],
	"night": ["국 다 떨어졌어. 내일 와."],
	"aff30": ["자네 밭 채소 좀 팔아. 값은 쳐 줄게.", "자네 요리 한 접시 갖다 줘. 평가해 줄게."],
	"aff70": ["주방을 자네한테 맡겨도 되겠어.", "내 국솥 비법을 자네한테만 말해 줄게."],
	"loves": ["fish_eel", "dish_grilled_fish"],
	"likes": ["egg", "milk"],
	"hates": ["sludge", "forage_trash"],
	"secret50": "읍 식당은 세 번 망했어. 이번이 네 번째야.\n망해도 국솥은 걸어. 그게 나야.",
	"secret100": "자네 할아버지가 내 첫 손님이었어. 국이 짜다고 했지.\n그 뒤로 소금을 반만 넣어. 지금 국이 그 맛이야.",
	},
	"fence_gu": {"name": "구씨", "birthday": [SUMMER, 13], "gender": "m", "romance": false,
	"lines": [
		"뭘 봐. 나는 그냥 여관 손님이야.",
		"물건 있으면 말해. 묻지 않고 사.",
		"장터에 못 내놓을 게 있으면 나한테 와.",
		"경찰서 쪽은 보지도 마. 나도 안 봐.",
		"값은 반이야. 싫으면 들고 가.",
	],
	"season": {
		SPRING: ["봄엔 물건이 없어. 다들 착해져."],
		SUMMER: ["여름밤엔 물건이 돌아."],
		FALL: ["가을엔 창고 물건이 많이 나와. 나는 몰라."],
		WINTER: ["겨울엔 여관 뒷방이 제일 따뜻해."],
	},
	"weather": {
		WEATHER_RAIN: ["비 오면 장사가 안 돼. 발자국이 남아서."],
		WEATHER_STORM: ["폭풍엔 아무도 안 와. 나도 자."],
		WEATHER_FOG: ["안개 낀 날이 좋아. 얼굴이 안 보여."],
	},
	"morning": ["아침부터 뭐야. 낮에 와."],
	"night": ["밤엔 뒷방이야. 문 두 번 두드려."],
	"aff30": ["자네 손버릇 얘기 들었어. 나쁘지 않아.", "물건은 값보다 사람을 봐. 자넨 믿어."],
	"aff70": ["나한테 파는 건 자네뿐이야. 그러니 값을 좀 더 쳐 주지.", "언젠가 이 장사도 접어야지. 그땐 자네한테 넘길까."],
	"loves": ["gem", "forage_relic"],
	"likes": ["forage_ring", "fish_eel"],
	"hates": ["dish_soup", "lamp"],
	"secret50": "옛날엔 순경이었어. 서장이랑 같이.\n물건 하나 잘못 만지고 이렇게 됐지. 서장은 아직도 나한테 인사해.",
	"secret100": "자네 할아버지 물건이 내 손을 거쳐 간 적 있어. 훔친 게 아니라 맡긴 거였어.\n그 물건, 아직 있어. 자네한테 돌려줘야지.",
	},
}
var affinity := {"librarian": 0,
	...
	"officer_park": 0,
	# 갈뫼읍 손글 아홉(S4c)
	"mayor_kang": 0, "chief_ha": 0, "judge_suh": 0, "pros_min": 0, "doctor_oh": 0,
	"manager_baek": 0, "innkeeper_ok": 0, "cook_jang": 0, "fence_gu": 0}
...
const JOBS := {
	...
	# ---- 갈뫼읍 기관직 넷(S4c) — 군청 서기 120 · 간호사 130 · 검사 250 · 판사 300(봉급 밴드) ----
	"county_clerk": {
		"name": "군청 서기",
		"kind": "office",
		"inst": "county",
		...
		"req_rep": 20,
		"slice": 4,
		"calls": {"stranger": "군청 새 사람", "known": "서기 양반", "master": "우리 서기"},
		"boss_calls": {
			"stranger": ["왔나. 군청 서류는 글씨가 반일세. 또박또박 쓰게.", "새 서기, 도장은 내가 찍네. 자넨 줄을 맞추게."],
			"known": ["서기 양반 왔구먼. 오늘 민원이 셋일세.", "서기 양반, 자네 글씨가 내 것보다 낫네."],
			"master": ["우리 서기 왔네. 군청이 자네 덕에 도네.", "우리 서기, 다음엔 과장 자리를 생각해 두게."],
		},
		"hire": {
			"ask": "군수님, 군청에서 일하고 싶습니다. 글은 좀 압니다.",
			"refuse_aff": "자네를 아직 모르네. 군청은 아는 사람만 들이네.",
			"refuse_rep": "교진 이장이 자네 얘기를 좋게 해야 하네. 아직은 아닐세.",
			"refuse_record": "전과가 있는 사람은 관청에 못 앉히네. 규정일세.",
			"refuse_busy": "일자리가 있잖나. 관청은 겸직이 안 되네.",
			"refuse_skill": "책을 세 권은 읽고 오게. 서류는 글일세.",
			"accept": "그래. 내일 아침 아홉 시에 군청으로 오게. 책상은 비워 두지.",
			"first_day": "첫날일세. 민원은 듣고, 적고, 넘기게. 판단은 내가 하네.",
			"resign_ask": "군수님, 군청 일을 그만두겠습니다.",
			"resign_reply": "그러게. 자네 글씨는 서류에 남네. 고마웠네.",
			"fired": "이레를 안 나왔어. 책상을 비워 둘 순 없네. 그만두게.",
		},
		"loop": [
			{
				"customer": "miller",
				"setup": "수길이 물레방아 수리비를 청구하러 왔다. 「군청이 좀 도와 준다던데.」",
				"choices": [
					["서류를 받아 군수에게 올린다", "수길: 고맙네. 물레가 서면 온 고장이 굶네.", 1],
					["규정에 없다고 돌려보낸다", "강 군수: 규정은 사람 살리라고 있네. 다시 받게.", -1],
					["절반만 된다고 말한다", "수길: 절반이라도 어딘가. 나머진 내가 대지.", 0],
				],
			},
			{
				"customer": "farmer",
				"setup": "순돌이 재산세 고지서를 들고 왔다. 「닭이 다섯인데 여섯으로 적혔네.」",
				"choices": [
					["고지서를 고쳐 준다", "순돌: 그렇지. 스무 닢이지만 스무 닢일세. 고맙네.", 1],
					["가축 수를 세러 가겠다고 한다", "순돌: 세러 온다고? 그럼 오게. 닭이 도망가진 않네.", 0],
					["낸 뒤에 돌려받으라고 한다", "순돌: 낸 돈이 돌아오는 걸 본 적이 없네.", -1],
				],
			},
		],
	},
	...
	"nurse": {
		...
		"req_rep": 20,
		"slice": 4,
		"calls": {"stranger": "보건소 새 사람", "known": "간호사 {ho}", "master": "우리 간호사"},
		"boss_calls": {
			"stranger": ["왔나. 손부터 씻게. 그다음이 환자일세.", "새 사람, 약은 이름부터 외우게."],
			"known": ["간호사 왔구먼. 오늘은 기절한 사람이 하나일세.", "간호사, 자네 손이 차분해서 좋네."],
			"master": ["우리 간호사 왔네. 자네가 있어 내가 진료를 보네.", "우리 간호사, 교진 진료소는 자네 몫일세."],
		},
		"hire": {
			"ask": "소장님, 보건소에서 일하고 싶습니다. 약초는 좀 압니다.",
			"refuse_aff": "자네를 아직 모르네. 환자를 모르는 사람한테 못 맡기네.",
			"refuse_rep": "마을이 자넬 믿어야 환자도 믿네. 아직은 아닐세.",
			"refuse_record": "전과가 있는 사람한테 약장을 못 맡기네.",
			"refuse_busy": "일자리가 있잖나. 보건소는 온종일일세.",
			"refuse_skill": "요리를 좀 더 해 보고 오게. 약도 끓이는 걸세.",
			"accept": "그래. 내일 아홉 시에 흰옷을 입고 오게.",
			"first_day": "첫날일세. 오늘은 환자 말만 듣게. 약은 내가 주네.",
			"resign_ask": "소장님, 보건소 일을 그만두겠습니다.",
			"resign_reply": "그러게. 자네 손은 기억할 걸세. 고마웠네.",
			"fired": "이레를 안 나왔어. 환자가 기다렸네. 그만두게.",
		},
		"loop": [
			{
				"customer": "foodie",
				"setup": "다미가 배를 붙잡고 왔다. 「신상 요리를 셋이나 먹었더니…」",
				"choices": [
					["따뜻한 물과 하루 금식을 권한다", "오 소장: 옳지. 약보다 쉬는 게 먼저일세.", 1],
					["약을 바로 준다", "다미: 약 먹고 또 먹어도 되죠? …안 되는구나.", -1],
					["무슨 요리였는지 적어 둔다", "다미: 적어 두면 뭐 해요. 그래도 고마워요.", 0],
				],
			},
			{
				"customer": "brook",
				"setup": "도담이 폭포에서 미끄러져 무릎이 까졌다. 「안 아파요. 진짜로.」",
				"choices": [
					["씻기고 붕대를 감아 준다", "도담: 붕대 멋있다. 내일 또 넘어질래요.", 1],
					["집에 가서 쉬라고 한다", "도담: 쉬면 폭포 못 보는데… 알겠어요.", 0],
					["별것 아니라고 돌려보낸다", "오 소장: 상처는 작아도 씻겨 보내게. 그게 일일세.", -1],
				],
			},
		],
	},
	"prosecutor": {
		...
		"req_aff": 30,
		"slice": 4,
		"calls": {"stranger": "검찰청 새 사람", "known": "검사님", "master": "우리 검사님"},
		"boss_calls": {
			"stranger": ["왔나. 서류부터 읽게. 사람은 그다음일세.", "새 검사, 증거가 문턱을 넘나부터 보게."],
			"known": ["검사님 왔군. 오늘 서류가 둘일세.", "검사님, 어제 불기소는 잘한 걸세."],
			"master": ["우리 검사님 왔네. 이제 서류를 나눠 들지.", "우리 검사님, 덮으라는 말이 오거든 나한테 말하게."],
		},
		"hire": {
			"ask": "부장님, 검사 자리를 맡고 싶습니다. 책은 열두 권 읽었습니다.",
			"refuse_aff": "자네를 아직 모르네. 서류를 맡기려면 사람을 알아야 하네.",
			"refuse_rep": "검사는 마을이 믿는 사람이라야 하네. 평판이 더 필요하네.",
			"refuse_record": "전과가 있는 사람이 남을 기소할 순 없네.",
			"refuse_busy": "일자리가 있잖나. 검사는 겸직이 안 되네.",
			"refuse_skill": "책 열두 권. 서하한테 가서 마저 읽고 오게.",
			"accept": "좋네. 내일 아홉 시. 서류는 내가 골라 두겠네.",
			"first_day": "첫날일세. 서류 하나에 답은 셋 — 기소, 불기소, 보완수사.",
			"resign_ask": "부장님, 검사 자리를 내려놓겠습니다.",
			"resign_reply": "그러게. 자네가 쓴 서류는 남네. 고마웠네.",
			"fired": "이레를 안 나왔어. 서류가 썩네. 내려놓게.",
		},
		"loop": [
			{
				"customer": "miner",
				"setup": "바우의 창고 사건 서류다. 본 사람 하나, 장물 없음. 「난 안 했소.」",
				"choices": [
					["보완수사를 지시한다", "민 부장: 옳지. 문턱에 못 미치면 더 보는 걸세.", 1],
					["기소한다", "민 부장: 본 사람 하나로 기소하나. 판사가 돌려보낼 걸세.", -1],
					["불기소한다", "바우: …고맙소. 다음엔 창고 근처도 안 가리다.", 0],
				],
			},
			{
				"customer": "carpenter",
				"setup": "덕구의 목재 도벌 서류다. 본 사람 둘, 판 기록 하나. 「내 나무였소.」",
				"choices": [
					["기소한다", "민 부장: 증거가 넘었네. 기소가 맞네.", 1],
					["덕구를 불러 먼저 듣는다", "덕구: 들어 주는 검사는 처음이오. …내 나무는 아니었소.", 0],
					["불기소한다", "민 부장: 본 사람 둘에 판 기록까지 있는데 덮나. 다시 보게.", -1],
				],
			},
		],
	},
	"judge": {
		...
		"req_aff": 30,
		"slice": 4,
		"calls": {"stranger": "법원 새 사람", "known": "판사님", "master": "우리 판사님"},
		"boss_calls": {
			"stranger": ["왔나. 판결문은 세 번 읽게. 한 번은 피고 눈으로.", "새 판사, 무거운 자리일세. 앉게."],
			"known": ["판사님 왔군. 오늘 사건이 둘일세.", "판사님, 어제 경감은 잘한 걸세."],
			"master": ["우리 판사님 왔네. 방청석에서 보겠네.", "우리 판사님, 윤 판사가 자네 얘길 하더군."],
		},
		"hire": {
			"ask": "부장님, 판사 자리를 맡고 싶습니다. 피고석에도 서 봤습니다.",
			"refuse_aff": "자네를 아직 모르네. 판사석은 아는 사람한테 주네.",
			"refuse_rep": "판사는 마을이 믿는 사람이라야 하네. 평판이 더 필요하네.",
			"refuse_record": "전과가 말소되지 않은 사람은 판사석에 못 앉네.",
			"refuse_busy": "일자리가 있잖나. 판사는 겸직이 안 되네.",
			"refuse_skill": "책 열두 권. 법은 글로 배우네.",
			"accept": "좋네. 내일 아홉 시. 법복은 맞춰 두지.",
			"first_day": "첫날일세. 피고 하나에 답은 셋 — 무죄, 경감, 법정형.",
			"resign_ask": "부장님, 판사 자리를 내려놓겠습니다.",
			"resign_reply": "그러게. 자네 판결문은 남네. 고마웠네.",
			"fired": "이레를 안 나왔어. 법정을 비워 둘 순 없네. 내려놓게.",
		},
		"loop": [
			{
				"customer": "painter",
				"setup": "나리가 피고석에 섰다. 창고에서 물감을 훔쳤다. 본 사람 둘. 「그림을 그리려고요.」",
				"choices": [
					["경감한다 — 봉사 사흘", "나리: …고마워요. 봉사 끝나면 그림 한 장 드릴게요.", 1],
					["법정형 — 벌금", "나리: 벌금은… 낼게요. 그림은 못 그리겠네요.", 0],
					["무죄로 한다", "서 부장: 본 사람이 둘인데 무죄인가. 판결문을 다시 쓰게.", -1],
				],
			},
			{
				"customer": "angler",
				"setup": "강돌이 피고석에 섰다. 남의 어망을 걷었다. 본 사람 하나. 「내 어망인 줄 알았소.」",
				"choices": [
					["무죄로 한다 — 증거 부족", "서 부장: 본 사람 하나면 무죄가 맞네. 잘 봤네.", 1],
					["경감한다 — 봉사 사흘", "강돌: 사흘이면… 하겠소. 어망은 돌려주겠소.", 0],
					["법정형 — 벌금", "서 부장: 하나로 벌금인가. 무겁네. 다시 읽게.", -1],
				],
			},
		],
	},
...
const SOCIETY_LINES := {
	...
	"fence": {
		"choice": "물건 넘기기",
		"none": "넘길 게 없구먼. 있거든 오게.",
		"ok": "됐네. %dG. 어디서 났는진 안 물어.",
		"gray": "장물이 없다.",
	},
