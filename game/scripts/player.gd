...
#
# 도구 그림은 한 장뿐이라(icon_*, 옆에서 본 모습) 앞·뒤를 볼 때 옆모습과 같은 호를 그리면
# 도구가 **옆으로 넘어간다** — 앞에서 캐는데 곡괭이가 오른쪽 땅을 찍었다. 앞·뒤는 호를
# 세로로 세운다: 감을 때 머리 위에서 살짝 뒤로 젖혔다가(-0.4) 내리칠 때 거의 곧장 아래(π)로,
# 주먹이 배 앞에 있으니 날이 발 앞 땅에 닿는다. 옆을 스치는 건 한 프레임뿐이다.
const SWING_POSE := {
	"down": {"hand": Vector2(10, -36), "mid": 1.35, "arc": 3.5, "tilt": 1.0,
		"shift": Vector2(1, 3), "spin": 1.0},
	"up": {"hand": Vector2(-9, -40), "mid": 1.35, "arc": 3.5, "tilt": 0.75,
		"shift": Vector2(1, -3), "spin": -1.0},
