...
| 파일 | 줄수 | 맡은 일 |
| --- | ---: | --- |
| `main.gd` | ~4500 | 세계 상태(grid·objects·player), 도구, 시간, 낮/밤, 그리기, 통신 |
| `story.gd` | ~1050 | 메인 스토리 연출 — 각본 |
| `dev_harness.gd` | ~940 | 검증 하네스 (KYOJIN_SHOT일 때만 붙는다) |
...
- **다른 스크립트의 옛 주소.** 도구는 `main.gd`만 고친다.
  `dev_harness.gd`가 `m._story_t`를 계속 보고 있어서, 스토리 하네스가 종료
  조건을 못 만나고 90초를 돌다 죽었다. → `python3 tools/check_refs.py`
...
- **통신** (`_net_*` · `_req_*` · `_broadcast_*`, ~520줄). `@rpc`는 노드 경로로
  찾아가므로 자식으로 옮기면 양쪽 peer의 경로가 같아야 한다. 하네스가
  멀티플레이를 안 돌리니 여기만은 검증 그물이 없다 — 뺄 거면 조심해서.
- **그리기** (`_draw*` · `_context_hint` · `nav_target`, ~450줄). `draw_*`는
  CanvasItem 메서드라 `m.draw_texture_rect(...)`로 불러야 하고, main의
  `_draw()` 안에서만 유효하다.
- **마을 대화** (상점·여관·연구소·도서관·축제·의뢰, ~460줄).
