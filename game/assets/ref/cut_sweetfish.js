// 은어(dmsdj) 교체 — 원본(투명 배경)의 여백을 잘라
// 32x32 격자(기존 fish_sweetfish.png 규격)에 꽉 차게 앉힌다.
// 가방·도감·낚시 연출이 이 한 장을 같이 쓴다.
// 다시 갈면 이 스크립트만 돌리면 된다. 원본: ref/src/ref_sweetfish.png
// 실행: repo 루트에서  NODE_PATH=... node game/assets/ref/cut_sweetfish.js
const fs = require('fs'), { PNG } = require('pngjs');
const im = PNG.sync.read(fs.readFileSync(__dirname + '/src/ref_sweetfish.png'));
...
fs.writeFileSync(__dirname + '/../sprites/fish_sweetfish.png', PNG.sync.write(out));
console.log('fish_sweetfish.png 32x32 완료 (bbox', bw, 'x', bh, ')');
