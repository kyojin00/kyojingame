// 유리 조각(dbfl) 교체 — 원본(투명 배경)의 여백을 잘라
// 32x32 격자(기존 forage_glass.png 규격)에 꽉 차게 앉힌다.
// 해변 채집 오브젝트·가방 아이콘이 이 한 장을 같이 쓴다.
// 다시 갈면 이 스크립트만 돌리면 된다. 원본: ref/src/ref_glass.png
// 실행: repo 루트에서  NODE_PATH=... node game/assets/ref/cut_glass.js
const fs = require('fs'), { PNG } = require('pngjs');
const im = PNG.sync.read(fs.readFileSync(__dirname + '/src/ref_glass.png'));
...
fs.writeFileSync(__dirname + '/../sprites/forage_glass.png', PNG.sync.write(out));
console.log('forage_glass.png 32x32 완료 (bbox', bw, 'x', bh, ')');
