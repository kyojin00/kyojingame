// 씨앗 아이콘(Tldkt) 교체 — 원본(641x641, 투명 배경)의 여백을 잘라
// 32x32 격자(기존 icon_seed.png 규격)에 꽉 차게 앉힌다.
// 모든 씨앗 일러스트(도구 아이콘·가방·상점)가 이 한 장을 같이 쓴다.
// 다시 갈면 이 스크립트만 돌리면 된다. 원본: ref/src/ref_seed.png
// 실행: repo 루트에서  NODE_PATH=... node game/assets/ref/cut_seed.js
const fs = require('fs'), { PNG } = require('pngjs');
const im = PNG.sync.read(fs.readFileSync(__dirname + '/src/ref_seed.png'));
...
fs.writeFileSync(__dirname + '/../sprites/icon_seed.png', PNG.sync.write(out));
console.log('icon_seed.png 32x32 완료 (bbox', bw, 'x', bh, ')');
