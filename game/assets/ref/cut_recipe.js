// 레시피 두루마리(fptl) — 모든 레시피 표시가 이 한 장을 같이 쓴다.
// 원본(투명 배경)의 여백을 잘라 32x32에 꽉 차게 앉힌다.
// 다시 갈면 이 스크립트만 돌리면 된다. 원본: ref/src/ref_recipe.png
// 실행: repo 루트에서  NODE_PATH=... node game/assets/ref/cut_recipe.js
const fs = require('fs'), { PNG } = require('pngjs');
const im = PNG.sync.read(fs.readFileSync(__dirname + '/src/ref_recipe.png'));
...
fs.writeFileSync(__dirname + '/../sprites/recipe.png', PNG.sync.write(out));
console.log('recipe.png 32x32 완료 (bbox', bw, 'x', bh, ')');
