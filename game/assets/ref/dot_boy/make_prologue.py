...
class Canvas:
    ...
    def bands(self, rows):
        """가로 색 띠 — [(끝 y, 색)]. 그라데이션 대신 평평한 띠로 하늘을 짜고,
        띠 경계는 체크무늬 두 줄로 섞어 계단을 부드럽게 만든다."""
        y = 0
        cols = []
        for y1, c in rows:
...
def scene_grandpa(g, f):
    ...
    g.p(hx + 1, hy, (250, 252, 255))                # 도톰한 머리
    g.p(hx, hy + 1, (190, 196, 230))
    # 달 — 또렷한 윤곽 + 숨쉬는 달무리
    mx, my, r = 137, 15, 8
    g.disk(mx, my, r + 3, mix((26, 26, 72), (120, 120, 150),
                              [0.40, 0.47, 0.42, 0.5][f]))
    ...
    # 밤구름 한 조각 — 달 앞을 천천히 지난다
    cwx = mx - 22 + f * 3
    for i in range(24):
        for j in range(2):
            x, y = cwx + i, 18 + j + (i // 9)
            if 0 <= x < W and abs(i - 12) + j * 3 < 13:
                g.px[x, y] = mix(g.px[x, y], (64, 64, 104), 0.5)
    # 뒷능선 (짙은 남보라) + 소나무
    BACK = (30, 26, 62)
    for x in range(W):
    ...
    g.pool(21, 48, 7, 5, (222, 174, 92),
           ((0.5, [0.4, 0.48, 0.34, 0.44][f]), (1.0, 0.2)))
    g.rect(20, 47, 22, 49, (244, 206, 120))         # 불 켜진 창 (깜빡임)
    if f == 2:
        g.p(21, 48, (222, 174, 92))
    # 앞 언덕 (더 짙게)
    HILL = (18, 14, 40)
    for x in range(W):
...
def scene_box(g, f):
    ...
    # 할아버지의 초상 액자 — 왼쪽 위 벽
    g.rrect(8, 6, 26, 26, (110, 80, 46), 2)
    g.rrect(11, 9, 23, 23, (52, 44, 40), 2)
    g.rect(14, 12, 20, 20, (198, 160, 130))         # 얼굴
    g.rect(14, 12, 20, 13, (232, 226, 212))         # 백발
    g.rect(15, 18, 19, 20, (232, 226, 212))         # 수염
    g.p(15, 15, (60, 44, 34))                       # 눈
    g.p(19, 15, (60, 44, 34))
    g.p(17, 5, (70, 52, 34))                        # 못
    # 창 — 밤하늘과 별
    ...
    for i in range(36):
        for wdt in range(8):
            ...
            y = 26 + i - wdt // 3
            if 0 <= x < W and 0 <= y < H:
                k = (0.2 if 2 <= wdt <= 5 else 0.11) * [1.0, 0.85, 1.12, 0.9][f]
                g.px[x, y] = mix(g.px[x, y], (150, 160, 210), k)
    ...
    for t in range(DEP - 4, DEP - 2):               # 속 뒷벽 — 촛불빛이 스민다
        y = by0 - round(t * RISE / DEP)
        g.rect(bx0 + t + 4, y, bx1 + t - 4, y, (64, 42, 28))
    ...
    for x in range(bx1, bx1 + DEP + 1):
        ...
        g.p(x, by0 - dy + 2, (122, 82, 44))         # 옆판 윗모서리 빛
        if (x - bx1) % 4 == 2:                      # 세로 잔결
            g.p(x, by0 - dy + 6 + (x - bx1), (92, 60, 34))
    ...
    for p in range(by0 + 1, by1 - 2, 7):            # 널마다 윗빛과 아랫그늘
        g.rect(bx0 + 3, p + 1, bx1 - 3, p + 1, (150, 104, 56))
        yk = min(p + 6, by1 - 1)
        g.rect(bx0 + 3, yk, bx1 - 3, yk, (126, 86, 46))
    for kx, ky in ((bx0 + 15, by0 + 11), (bx0 + 31, by0 + 25),
                   (bx0 + 24, by0 + 4)):            # 옹이
        g.p(kx, ky, (108, 72, 38))
        g.p(kx + 1, ky, (96, 62, 34))
        g.p(kx, ky + 1, (96, 62, 34))
        g.p(kx + 1, ky + 1, (120, 82, 44))
    for _ in range(18):                             # 잔결
        gx = rnd.randrange(bx0 + 4, bx1 - 4)
        gy = rnd.randrange(by0 + 2, by1 - 2)
        g.rect(gx, gy, gx + rnd.randrange(2, 4), gy, (132, 90, 48))
    for y in range(by0 + 7, by1 - 1, 7):            # 널마다 못 두 개
        g.p(bx0 + 7, y + 3, (88, 58, 32))
        g.p(bx1 - 7, y + 3, (88, 58, 32))
    g.rect(bx0 + 1, by1, bx1 - 1, by1, (118, 80, 42))   # 밑단 그늘 마감
    for x0 in (bx0, bx1 - 4):                       # 앞판 쇠장식 + 리벳
        g.rrect(x0, by0, x0 + 4, by0 + 5, (128, 130, 138), 2)
        g.rrect(x0, by1 - 5, x0 + 4, by1, (104, 106, 114), 2)
        g.p(x0 + 2, by0 + 2, (176, 178, 186))
        g.p(x0 + 1, by0 + 4, (86, 88, 96))
        g.p(x0 + 3, by0 + 4, (86, 88, 96))
        g.rect(x0, by0 + 5, x0 + 4, by0 + 5, (94, 96, 104))
        g.p(x0 + 2, by1 - 3, (150, 152, 160))
        g.p(x0 + 1, by1 - 1, (78, 80, 88))
    ...
    # 편지 봉투 — 상자 앞에 비스듬히 기대 바닥에 서 있다
    ev0, ev1, et, eb = 74, 100, 48, 62
    ...
    for i in range(13):                                         # 봉투 접힌 V
        y = et + 1 + i // 2
        off = (eb - y) // 7
        g.p(ev0 + 1 + i + off, y, (198, 182, 148))
        g.p(ev1 - 1 - i + off, y, (198, 182, 148))
    g.disk(87, et + 9, 2, (170, 48, 42))                        # 밀랍 인장
    g.p(86, et + 8, (206, 82, 68) if f % 2 else (184, 62, 54))
    ...
    for i in range(5):
        dx_ = 34 + (i * 19 + f * 2) % 58
        dy_ = 12 + (i * 11 - f + 40) % 34
        g.p(dx_, dy_, (178, 150, 106))
    # 나방 — 촛불 곁을 맴돈다
    mo = [(40, 9), (43, 12), (39, 14), (36, 10)][f]
    g.p(mo[0], mo[1], (214, 204, 178))
    g.p(mo[0] + (-1 if f % 2 else 1), mo[1] - (f % 2), (168, 158, 136))
    # 바닥 소품 — 열쇠와 동전 (촛불 빛 웅덩이 안, 상자 앞 바닥)
    g.rect(48, 64, 52, 64, (170, 150, 100))
    g.p(53, 63, (170, 150, 100))
    g.p(54, 64, (170, 150, 100))
    g.p(49, 65, (136, 118, 78))
    g.p(57, 67, (204, 174, 94))
    g.p(58, 67, (228, 198, 112))
...
def scene_letter(g, f):
    ...
    g.p(33, 57, (206, 82, 68) if f % 2 else (190, 64, 54))
    # 눌린 꽃잎
    ...
    # 종이 귀퉁이 말림 — 오른쪽 아래가 살짝 들려 그늘이 진다
    for i in range(6):
        g.p(143 - i, 65 - i // 2, (214, 196, 158))
        g.p(144 - i, 65 - i // 2, (198, 178, 140))
    g.p(143, 64, (250, 242, 214))                   # 들린 귀 하이라이트
    g.p(144, 65, (250, 242, 214))
    ...
    g.pool(150, 4, int(66 * br), int(46 * br), (255, 196, 96),
           ((0.2, 0.36), (0.4, 0.28), (0.6, 0.2), (0.8, 0.13), (1.0, 0.07)))
    for i in range(4):                              # 촛불 빛 속 먼지
        dx_ = 118 + (i * 17 + f * 2) % 44
        dy_ = 6 + (i * 13 - f + 30) % 26
        g.p(dx_, dy_, (198, 172, 122))
...
def scene_farm(g, f):
    ...
    g.p(89, 54 - (f % 2), (232, 186, 100))          # 깃발이 바람에 까딱인다
    # 갈대 — 왼쪽 가장자리
    ...
    for bi, (bx, by) in enumerate(((100, 12), (110, 9))):
        ...
        if wing_up:
            ...
            g.p(bx + 1, by, (40, 34, 48))
    # 해의 빛 웅덩이 — 지평선과 들판에서 숨쉬듯 번진다
    sbr = [1.0, 1.08, 0.94, 1.04][f]
    g.pool(sx, sy, int(44 * sbr), int(26 * sbr), (255, 200, 100),
           ((0.2, 0.36), (0.4, 0.28), (0.6, 0.2), (0.8, 0.13), (1.0, 0.07)))
    g.pool(sx, 46, int(54 * sbr), int(12 * sbr), (255, 190, 90),
           ((0.3, 0.24), (0.6, 0.16), (1.0, 0.09)))
    # 반딧불이 — 어스름 들판
