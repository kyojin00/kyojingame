...
def scene_box(g, f):
    ...
    # 달빛 줄기 — 창에서 왼쪽 아래로 비껴 내리는 평평한 띠
    for i in range(34):
        for wdt in range(10):
            x = 118 - i + wdt
            y = 26 + i - wdt // 3
            if 0 <= x < W and 0 <= y < H:
                g.px[x, y] = mix(g.px[x, y], (150, 160, 210), 0.13)
    # 바닥 — 금빛 널마루 (쨍한 주황 골드)
    for y in range(42, H):
    ...
    # 나무 상자 — 검은 윤곽선을 두른 또렷한 소품
    bx0, bx1 = 60, 112
    by0, by1 = 30, 58
    g.rrect(bx0 + 2, by0 - 10, bx1 - 2, by0 - 1, (96, 64, 38), 3)   # 뚜껑
    g.rect(bx0 + 4, by0 - 10, bx1 - 4, by0 - 9, (66, 44, 28))
    g.rrect(bx0, by0, bx1, by1, (140, 96, 50), 3)
    for y in range(by0 + 7, by1, 7):
    ...
    for x in range(bx0 - 1, bx1 + 2):
        g.p(x, by1 + 1, OUTLINE)
        g.p(x, by0 - 11, OUTLINE)
    for y in range(by0 - 10, by1 + 1):
        g.p(bx0 - 1, y, OUTLINE)
        g.p(bx1 + 1, y, OUTLINE)
    g.rect(bx0 + 2, by0 - 1, bx1 - 2, by0 + 2, (40, 26, 18))   # 속 그늘
    # 연구 노트 (상자에 기대 있다)
    g.rrect(72, 20, 100, 30, (74, 48, 32), 2)
    g.rect(74, 20, 98, 21, (98, 66, 42))
    g.rect(77, 20, 79, 30, (154, 110, 48))          # 가죽끈
    g.p(93, 23, (150, 130, 96))                     # 모서리 장식
    g.p(94, 23, (150, 130, 96))
    # 편지 봉투 + 밀랍 인장
    g.rrect(76, 32, 104, 44, (238, 226, 196), 2)
    g.rect(78, 32, 102, 33, (250, 244, 222))
    for i in range(14):
        g.p(77 + i, 33 + i // 2, (198, 182, 148))
        g.p(103 - i, 33 + i // 2, (198, 182, 148))
    g.disk(90, 40, 1, (170, 48, 42))
    g.p(89, 39, (206, 82, 68) if f % 2 else (184, 62, 54))
    # 촛불 — 왼쪽. 도트가 굵어 불꽃 한 알 한 알이 보인다
    g.rrect(30, 22, 35, 40, (228, 214, 186), 2)
    g.rect(31, 22, 34, 23, (246, 238, 212))
    g.p(30, 27, (208, 194, 166))                    # 촛농
    g.p(35, 32, (208, 194, 166))
    g.rrect(26, 40, 39, 43, (146, 132, 112), 2)
    flame = [((32, 18), (32, 19), (33, 20)),
             ((32, 17), (32, 18), (32, 19)),
             ((33, 18), (32, 19), (32, 20)),
             ((32, 18), (33, 19), (32, 20))][f]
    ...
    # 촛불 빛 웅덩이 — 벽과 바닥에 띠로 진다 (프레임마다 살짝 숨쉰다)
    br = [1.0, 1.12, 0.92, 1.06][f]
    g.pool(32, 20, int(34 * br), int(24 * br), (255, 190, 90),
           ((0.35, 0.4), (0.65, 0.25), (1.0, 0.12)))
    g.pool(46, 58, int(52 * br), int(15 * br), (255, 190, 90),
           ((0.4, 0.3), (0.75, 0.18), (1.0, 0.09)))
    # 나방 — 촛불 곁을 맴돈다
    mo = [(40, 14), (43, 17), (39, 19), (36, 15)][f]
    ...
    # 탁자 소품 — 열쇠와 동전
    g.rect(120, 50, 124, 50, (170, 150, 100))
    g.p(125, 49, (170, 150, 100))
    g.p(126, 50, (170, 150, 100))
    g.p(121, 51, (136, 118, 78))
    g.p(132, 53, (204, 174, 94))
    g.p(133, 53, (228, 198, 112))
