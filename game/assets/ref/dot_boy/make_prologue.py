...
class Canvas:
    ...
    def rrect(self, x0, y0, x1, y1, c, r=2):
        """모서리를 깎은 사각형 — r=2 는 꼭짓점 한 칸, r=3 은 세 칸을 깎아
        도트 소품이 덜 각져 보인다."""
        for y in range(max(0, y0), min(H, y1 + 1)):
            for x in range(max(0, x0), min(W, x1 + 1)):
                dx = min(x - x0, x1 - x)
...
def scene_box(g, f):
    ...
    for y in range(7, 12):
        g.p(38, y, (50, 36, 26))
    # 벽 액자 — 할아버지의 작은 초상
    g.rrect(196, 16, 226, 44, (96, 70, 42), 3)
    g.rrect(199, 19, 223, 41, (56, 48, 40), 2)
    g.rrect(206, 24, 216, 34, (196, 158, 128), 2)   # 얼굴
    g.rect(207, 24, 215, 26, (230, 224, 210))       # 백발
    g.rrect(208, 31, 214, 34, (230, 224, 210), 2)   # 수염
    g.p(209, 28, (60, 44, 34))
    g.p(213, 28, (60, 44, 34))
    # 탁자
    for y in range(58, H):
    ...
    # 두루마리 지도통 (상자 왼쪽)
    g.rrect(52, 78, 78, 84, (110, 82, 48), 2)
    g.rect(54, 78, 76, 79, (134, 102, 60))
    g.rrect(50, 77, 53, 85, (86, 62, 38), 2)
    g.rrect(77, 77, 80, 85, (86, 62, 38), 2)
    # 나무 상자 — 뚜껑이 뒤로 젖혀져 열려 있다 (모서리는 둥글게 깎는다)
    bx0, bx1 = 86, 166
    by0, by1 = 44, 84
    g.rrect(bx0 + 4, by0 - 16, bx1 - 4, by0 - 2, (96, 66, 38), 3)
    g.rect(bx0 + 6, by0 - 16, bx1 - 6, by0 - 14, (70, 48, 28))
    for x in range(bx0 + 8, bx1 - 4, 12):           # 뚜껑 널빤지
        for y in range(by0 - 13, by0 - 2):
            g.p(x, y, (82, 56, 32))
    g.rrect(bx0, by0, bx1, by1, (134, 96, 54), 4)
    for y in range(by0, by1, 9):
        g.rect(bx0 + (2 if y in (by0,) else 0), y,
               bx1 - (2 if y in (by0,) else 0), y, (108, 76, 42))
    ...
    for x0 in (bx0, bx1 - 5):                       # 모서리 쇠장식 + 못
        ...
        g.rrect(x0, by1 - 6, x0 + 5, by1, (96, 98, 104), 3)
        g.p(x0 + 2, by0 + 2, (168, 170, 176))
        g.p(x0 + 2, by1 - 3, (140, 142, 148))
    g.rect(bx0 + 4, by0 - 1, bx1 - 4, by0 + 3, (34, 22, 16))
    # 연구 노트
    g.rrect(104, 30, 148, 46, (70, 46, 30), 3)
    g.rect(106, 30, 146, 32, (94, 62, 40))
    g.rect(111, 30, 114, 46, (150, 108, 46))
    g.p(112, 40, (108, 76, 34))
    g.p(113, 40, (108, 76, 34))
    g.rrect(142, 33, 145, 36, (150, 130, 96), 2)
    g.rect(122, 38, 136, 39, (120, 92, 60))
    # 편지 봉투 + 밀랍 인장
    g.rrect(112, 48, 156, 66, (226, 214, 184), 3)
    g.rect(114, 48, 154, 49, (244, 236, 212))
    for i in range(22):
    ...
    g.rrect(54, 30, 61, 52, (222, 210, 184), 2)
    g.rect(55, 30, 60, 31, (240, 232, 208))
...
def scene_letter(g, f):
    ...
    g.rrect(12, 8, 20, 14, (30, 34, 46), 2)
    g.rect(13, 8, 19, 9, (74, 80, 98))
...
def scene_farm(g, f):
    ...
    # 오두막 — 모서리를 깎아 둥글린다
    hx0, hy0 = 78, 34
    g.rrect(hx0, hy0 + 12, hx0 + 44, hy0 + 36, (150, 108, 62), 3)
    ...
    g.rrect(hx0 + 18, hy0 + 24, hx0 + 27, hy0 + 37, (96, 62, 36), 3)  # 문 (윗모서리 둥글게)
    g.rect(hx0 + 20, hy0 + 24, hx0 + 25, hy0 + 25, (76, 48, 30))
    g.p(hx0 + 25, hy0 + 30, (150, 130, 96))
    g.rrect(hx0 + 33, hy0 + 20, hx0 + 40, hy0 + 27, (58, 62, 74), 2)
    ...
    g.rrect(138, 82, 156, 88, (134, 102, 60), 3)
    g.rect(140, 82, 154, 83, (154, 120, 72))
