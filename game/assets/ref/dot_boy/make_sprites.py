...
SWING = {
    # 앞모습: 옆으로 쓸지 않고 **화면 쪽으로** 내리친다 (원근). 왼쪽 위로
    # 감아올린 도구가 머리 뒤로 넘어갔다가(주먹이 정수리 위로 빼꼼) 몸 앞
    # 가운데로 떨어진다 — 내리친 주먹은 배 앞이라 팔이 짧아 보인다(단축법).
    'down': {'skip': 'left', 'shoulder': (9, 21),
             'poses': [((5, 16), (5, 19), -2, 0, False),
                       ((3, 7), (2, 14), -3, 0, False),
                       ((12, 0), (9, 8), 0, 0, True),
                       ((15, 23), (11, 24), 1, 2, False),
                       ((16, 20), (12, 21), 0, 0, False)]},
...
def arm_stroke(g, x0, y0, fx, fy, ex, ey):
    ...
    for (x, y) in cells:                       # 획 둘레 윤곽선
        for nx, ny in ((x - 1, y), (x + 1, y), (x, y - 1), (x, y + 1)):
            if (nx, ny) in cells or not (0 <= nx < GW and 0 <= ny < GH):
                continue
            # 어깨 언저리는 두르지 않는다 — 여기까지 두르면 팔이 몸통에서
            # 잘려 붙인 막대처럼 뜬다. 어깨에서는 셔츠가 그대로 이어진다.
            if (nx - x0 - 1) ** 2 + (ny - y0 - 1) ** 2 <= 10:
                continue
            if g.d[ny][nx] != '.':
                g.d[ny][nx] = 'O'
    for (x, y) in cells:
