...
# 보폭을 2·3에서 **3·4**로 키웠다. 두 신발이 5칸씩이라, 발 사이가 5칸
# 벌어져도 신발이 맞닿고 6~7칸이면 남는 한 칸을 윤곽선이 메워 버린다 —
# 재 보니 여섯 칸 **전부** 두 발 사이에 빈 칸이 하나도 없었다. 걷는데
# 다리가 한 덩이로 붙어 보인 게 그래서다. 배경이 한 칸이라도 비치려면
# 발 중심이 아홉 칸은 떨어져야 한다.
STRIDE = [0, 3, 4, 0, -3, -4]
...
# f0에선 왼다리가 나가는 참(다음 칸이 +2), f3에선 반대다.
#
# **옆모습에는 안 쓴다.** 옆에서 본 통과 자세는 원래 다리가 하나로 겹쳐
# 보이는 게 맞다. 거기서 한쪽을 들면 신발이 위아래로 포개져 부러진 다리가
# 된다 — 옆모습의 두 통과 칸은 다리가 같고 **팔**로 갈린다.
PASS = [1, 0, 0, -1, 0, 0]
# 옆모습 팔이 앞으로 나간 양. 다리와 반대로, 다리보다 한 칸 크게 젓는다.
#
# 다리 위상을 그대로 뒤집으면 통과 칸에서 팔도 0이 돼 f0과 f3이 **같은
# 그림**이 된다. 그래서 위상을 밀어 통과 칸에도 ±2를 준다. ±1로 해 봤더니
# 두 칸이 도트 세 개만 달라 사실상 같은 그림이었다.
#
# 그래서 팔은 한 바퀴 내내 수직을 지나지 않는다 — 그게 맞다. 팔이 곧게
# 내려온 그림은 「서 있는 사람」이고, 걷는 사람의 팔은 늘 어딘가로 가는
# 중이다. -4에서 +2로 한 칸에 건너뛰는 것도 맞다: 진자는 아래를 지날 때
# 제일 빠르다.
ARM = [-2, -4, -5, 2, 4, 5]
...
def legs_side(g, stride, lean=0, dx=0, sq=0):
    ...
    ── 세 번 고쳐 그리고 나서 알아낸 것 ──────────────────────────────
    옆모습 다리가 계속 괴이했던 건 굵기나 보폭이 아니라 **폭이 줄마다
    달라서**였다. 허벅지 4칸 → 종아리 3칸 → 신발 5칸으로 잡아 뒀는데,
    거기에 줄마다 x가 한 칸씩 밀리는 기울기까지 겹치니 다리 윤곽이
    지그재그 계단이 됐다. 무릎께가 잘록하고 발목께가 다시 불거지는,
    사람 다리가 아닌 모양.

    더 나빴던 건 먼 다리다. 보폭이 ±2일 때 두 다리 중심이 겨우 두 칸
    떨어지는데 다리가 4칸이라, 먼 다리는 가까운 다리에 거의 다 가려
    **한 칸짜리 성냥개비**만 남았다. 그런데 발만은 off*1.35로 더 멀리
    보내 놨으니, 성냥개비 끝에 6칸짜리 신발이 달린 꼴이 됐다.

    그래서 규칙을 셋으로 줄였다.
      1. 다리는 **위아래 같은 굵기 4칸**. 발목만 신발이라 5칸.
      2. 엉덩이·무릎·발이 하나의 비율(0.30 / 0.80 / 1.15)로 함께 나간다.
         발만 따로 멀리 보내지 않는다 — 그래야 신발이 다리 끝에 붙는다.
      3. 보폭 0에서는 두 다리가 **완전히 겹친다**. 그게 맞다 — 옆에서
         본 통과 자세는 원래 다리 하나로 보인다. 억지로 한쪽을 들어
         비켜 세우려다 신발이 위아래로 포개져 부러진 다리가 됐다.
         두 통과 칸(f0·f3)은 다리가 같고 **팔이 다르다**.
    """
    c = 15 + lean
    # 바지 띠 — 몸통(10칸)보다 한 칸씩 안으로 들어간 8칸. 셔츠 밑단이
    ...
    # 띠와 다리 사이 단차를 그늘로 눌러, 다리가 그늘 속에서 나오게 잇는다.
    # 먼 다리를 그늘색으로 먼저, 가까운 다리를 위에 얹는다.
    for off, shade in ((-stride, True), (stride, False)):
        # 뒤로 간 발은 뒤꿈치가 들린다. 보폭이 클 때만 확실히 든다 —
        # 두 발이 가까운 칸에서 한쪽만 들면 신발이 포개져 지저분하다.
        # 뒤로 간 발은 뒤꿈치를 확실히 든다. 옆에서 본 걸음에서 두 발을
        # 가르는 단서는 좌우 간격만이 아니다 — **하나는 딛고 하나는 떠
        # 있다**는 높이 차가 그 못지않게 크다. 짧은 다리로는 좌우로
        # 벌릴 수 있는 폭에 한계가 있으니 이쪽을 같이 써야 한다.
        lift = 3 if off <= -3 else (2 if off < 0 else 0)
        pc, kc = ('P', 'n') if shade else ('p', 'k')
        bot = GROUND - lift
        hip_row = HIP_Y + 2 + sq
        knee_row = (hip_row + bot) // 2
        ankle_row = bot - 2                    # 아래 두 줄이 신발
        # 골반은 조금, 무릎부터 발까지는 크게 나간다.
        #
        # **무릎 아래는 수직**이다 — 정강이와 신발이 같은 열에 선다.
        # 예전엔 무릎 0.80 / 발 1.15로 발을 더 내보냈는데, 그 0.35칸 차이가
        # 반올림에서 한 칸이 되어 정강이와 신발이 한 칸 어긋났다. 두 다리가
        # 겹칠 때 그 한 칸이 「정강이는 2칸인데 신발은 4칸」으로 벌어져
        # 다리가 뚝 끊겨 보였다. 실제 걸음에서도 디딤 구간의 정강이는
        # 거의 곧게 서 있으니, 붙여 두는 편이 그림도 사실도 낫다.
        hip_x = off * 0.35
        knee_x = foot_x = off * 1.35
        if shade and not stride:
            # 보폭 0(정지·통과 칸)에서만 먼 다리를 한 칸 뒤로 물린다.
            # 정확히 겹치면 먼 다리가 **통째로 사라져** 외다리로 서 있는
            # 그림이 되기 때문이다. 보폭이 있는 칸에서는 물릴 필요가 없고,
            # 물리면 앞뒤가 비대칭이 되어 f1과 f4가 서로 달라진다.
            hip_x -= 0.7
            knee_x = foot_x = knee_x - 0.7
        # 다리 앞쪽 굵기. 먼 다리만 한 칸 더 둔다.
        #
        # 다리를 뒤 2칸·앞 1칸으로 잡아 두면, 먼 다리가 **뒤에** 있을 땐
        # 뒤쪽 2칸이 남지만 **앞에** 설 때는 앞쪽 1칸만 남는다. 그래서
        # 보폭 +2(f1)는 멀쩡한데 -2(f4)에서만 다리가 성냥개비가 됐다.
        # 뒤에 있는 다리를 앞뒤 2칸씩 대칭으로 두면 어느 쪽에 서든 같은
        # 굵기가 남는다. 굵어진 만큼은 어차피 가까운 다리에 가린다.
        ...
        for yy in range(LEG_Y - 1 + sq, bot + 1):   # 띠 아래 줄부터 겹쳐 잇는다
            ...
            # 허벅지는 굵고 무릎으로 갈수록 가늘어진다.
            #
            # 위아래 같은 굵기 4칸으로 통일했더니 계단은 사라졌는데 이번엔
            # **바지 띠(8칸) 밑에 막대 두 개를 꽂아 둔 꼴**이 됐다. 8칸에서
            # 4칸으로 한 줄 만에 떨어지니 엉덩이 양옆에 각진 어깨가 생기고,
            # 허벅지라는 게 아예 없는 다리가 된 것이다.
            #
            # 그래서 허벅지 여섯 줄에 걸쳐 6칸 → 5칸 → 4칸으로 내린다.
            # 띠 끝과 한 칸 차이로 시작해 무릎에서 정강이 굵기에 닿으니,
            # 골반에서 다리가 흘러나오는 것처럼 이어진다. 무릎 아래는
            # 그대로 4칸 — 정강이는 원래 곧다.
            boot = yy > ankle_row
            if boot:
                # 신발도 정강이와 **같은 앞폭**으로. 다르면 가림선 자리가
                # 줄마다 옮겨 다녀 먼 다리에 층이 생긴다 (밑창과 앞코가
                # 한 칸씩 더 나가는 건 아래에서 따로 붙인다).
                wb, wf = 2, wf_leg
            elif yy <= knee_row:
                f = (yy - hip_row) / max(1, knee_row - hip_row)   # 0=골반 1=무릎
                wb = 4 if f < 0.2 else 3 if f < 0.7 else 2
            ...
            if not boot and hip_row < yy:
                ...
                if not shade:
                    g.px(x + wf, yy, 'q')      # 가까운 다리 앞쪽 하이라이트
            if yy == ankle_row:
                # 발목 접단 — 밝게. 바지도 신발도 어두운 갈색이라 여기가
                # 어두우면 정강이부터 발끝까지 한 덩어리가 된다.
                g.rect(x - wb, yy, x + wf, yy, 'q' if not shade else 'p')
            if yy == knee_row and off < 0:
                g.px(x + wf, yy, 'P' if not shade else pc)   # 굽힌 무릎 자국
            if yy == bot:
                g.hline(x - wb, x + wf + 1, yy, 'K')         # 밑창은 늘 그늘
            # 가까운 다리에서 **먼 다리와 맞닿는 쪽** 모서리를 어둡게 눌러
            # 둘을 뗀다. 둘 다 바지색이라 겹치면 한 덩어리로 보인다.
            #
            # 어느 쪽인지는 보폭 부호가 정한다. 늘 뒤쪽에만 그었더니,
            # 가까운 다리가 **뒤로 간 칸**(보폭이 음수)에서는 먼 다리가
            # 앞쪽에 있는데 선은 엉뚱한 뒤에 그어져, 두 다리가 통째로
            # 붙은 굵은 기둥 하나가 됐다.
            if not shade and stride:
...
def frame(direction, stride=None, bob=0, pass_leg=0, arm=None):
    ...
    if direction == 'side':
        ...
        legs(g, s, lean, 0, bob)
        torso(g, bob, s, lean, arm=arm)
        head(g, art, bob, lean)
    else:
        # 앞뒤 모습은 두 다리가 나란해 겹치지 않으니 옆모습만큼 크게 벌릴
        # 이유가 없다. 게다가 정면은 보폭을 **들어 올리는 양**으로만
        # 표현하는데(min(3, -s)) 상한이 3이라, 보폭 3과 4가 같은 그림이 된다.
        # 4분의 3으로 줄이면 예전 값 [0,2,3,0,-2,-3] 그대로 — 앞뒤 걸음은
        # 손대지 않은 채로 둔다.
        sf = round(s * 0.75)
        legs(g, sf, 0, bob, pass_leg)
        torso(g, bob, sf)
        head(g, art, bob)
    roughen(g)
    g.outline()
...
def render_set(heads, blinks, ref_prefix, out_prefix):
    ...
    for d in ('down', 'side', 'up'):
        ...
        for i in range(WALK):
            images[f'{d}_walk_{i}'] = frame(
                d, STRIDE[i], BOB[i], PASS[i], ARM[i] if d == 'side' else None).render()
