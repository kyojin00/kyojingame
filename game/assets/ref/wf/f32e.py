...
# 옆얼굴 — **앞모습과 같은 수치**로 맞춘다. 재 보니 옆모습 머리가 15칸,
# 몸통이 10칸인데 앞모습은 18칸·18칸이었다. 몸통이 절반이면 방향이 바뀔
# 때마다 캐릭터가 홀쭉해졌다 부풀었다 한다.
# 머리 1~8줄(정수리)은 앞모습 표를 그대로 쓰고, 9줄부터만 앞쪽을 얼굴에
# 맞춰 깎는다. 몸통은 14칸(앞 16칸), 바지는 10칸(앞과 같다).
HEAD_SIDE = {
    1: (11, 22), 2: (10, 23), 3: (9, 24), 4: (8, 24), 5: (7, 25),
    6: (7, 25), 7: (6, 25), 8: (6, 25), 9: (6, 24), 10: (6, 24),
    11: (6, 24), 12: (6, 24), 13: (6, 24), 14: (7, 25), 15: (7, 24),
    16: (8, 23), 17: (9, 22), 18: (10, 21), 19: (11, 20), 20: (12, 20),
}
...
# 머리는 16줄에서 끝난다. 17~19줄에 한 칸짜리 머리가 남으면 뺨을 타고
# 내려가는 줄이 된다 — 그 아래는 목이니 살결이 실루엣을 꽉 채운다.
FACE_SIDE_BOY.update({15: (12, 23), 16: (12, 22), 17: (10, 22),
                      18: (11, 21), 19: (12, 20)})
...
def draw_head_side(g, girl):
    ...
    # 앞머리가 이마를 덮고 관자놀이로 흘러내린다. 끝 줄을 두 칸씩 묶어
    # 놨더니 머리선이 두 칸짜리 계단이 되어 톱니처럼 보였다 — 칸마다
    # 한 줄씩 내려 완만한 사선으로 만든다.
    for x, bot in ((12, 16), (13, 15), (14, 14), (15, 13), (16, 12),
                   (17, 11), (18, 10), (19, 10), (20, 9)):
        for y in range(6, bot + 1):
            swap(g, x, y, 'sl', 'h')
    rim_prof(g, 7, PROF_SIDE, 'H')
    strand(g, 12, 2, 6)
    strand(g, 19, 1, 3)
    # 그늘 띠가 두 칸인데 목덜미 머리가 두 칸뿐이라 통째로 먹혀 뺨을 타고
    # 내려가는 연필 자국이 됐다 — 남자는 14줄에서 멈춘다. 그리고 얼굴과
    # 맞닿는 10줄 아래는 한 칸만 어둡게: 두 칸이면 굵은 사선 줄무늬가 된다.
    rim_right(g, 3, 9, 2, 'd')
    rim_right(g, 10, 19 if girl else 14, 1, 'd')
    ...
    hfill(g, 17, 19, 20, 'm')      # 입 — 두 칸 (세 칸이면 옆얼굴에서 길다)
    ear = 14 if girl else 13       # 귀는 볼 뒤끝에 붙는다
    put(g, ear, 16, 'S')
    put(g, ear + 1, 16, 'S')
    hfill(g, 19, 16, 20, 'S')      # 턱 그늘 — 턱 밑 앞쪽만. 한 줄을
                                   # 통째로 칠하면 목에 굵은 띠가 생긴다
    hfill(g, 20, 13, 19, 'k')      # 목 — 턱에서 한 칸만 들어간다. 두 칸을
    hfill(g, 20, 14, 18, 'S')      # 들여 놨더니 턱이 깃에 바로 얹혔다
...
def body_side(g, girl):
    ...
    BODY = {21: (11, 20), 22: (10, 20)}
    for y in range(23, 32):
        BODY[y] = (10, 20)          # 몸통 아홉 칸 — 뒤로 한 칸이 엉덩이다
    ...
    for y in range(25, 32):        # 등 쪽 한 단 어둡게 — 앞뒤가 갈린다
        swap(g, 11, y, 't', 'y')
    for y in range(23, 27):        # 팔 — 몸통 한가운데, 다섯 칸
        hfill(g, y, 13, 17, 'T')
    for y in range(23, 32):        # 뒤쪽 이음선 — 팔이 몸 앞에 있다는 표시
        swap(g, 12, y, 't', 'y')
    hfill(g, 27, 13, 17, 'U')      # 소맷단
    for y in (28, 29):             # 손 — 두 단에 걸쳐 오므린다
        hfill(g, y, 13, 17, 's')
    for y in (30, 31):
        hfill(g, y, 14, 17, 's')
    hfill(g, 31, 14, 17, 'S')
    hfill(g, 31, 11, 19, 'Y')      # 윗도리 밑단
    if girl:
        hfill(g, 32, 11, 20, 'Q')
...
def build_side(girl):
    ...
    if girl:
        ...
        for y in range(14, 21):            # 결 — 길이가 다른 두 도막.
            swap(g, 7, y, 'hH', 'd')       # 한 줄로 쭉 그으면 판자가 된다
        for y in range(23, 28):
            swap(g, 8, y, 'hH', 'd')
    body_side(g, girl)
    if girl:
