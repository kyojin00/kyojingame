...
def build_boy():
    ...
    hfill(g, 22, 11, 20, 'T')      # 어깨 뚜껑 — 빛을 정면으로 받는 면
    hfill(g, 23, 9, 22, 'T')       # 두 줄을 깔고 그 아래에서 팔만 남긴다
    for y in range(24, 28):
        hfill(g, y, 8, 10, 'T')
    for y in range(24, 32):        # 오른팔 이음선 한 줄 (오른팔 색은 몸통과 같다)
        swap(g, 20, y, 't', 'y')
    for y in (24, 25):             # 왼쪽 겨드랑이 주름
        swap(g, 11, y, 't', 'y')
