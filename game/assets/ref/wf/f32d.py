...
def draw_head(g, HEAD, BANG, lit_cols, dark_rows, eye_top,
    ...
    put(g, EL, t, 'W')                   # 반사점은 **두 눈 모두 왼쪽 위**.
    put(g, ER, t, 'W')                   # 좌우 대칭이면 사시로 보인다
    if lash:
        put(g, EL, t + 2, 'e')
        put(g, ER + 2, t + 2, 'e')
    ...
    if blush_wide:
        swap(g, 12, cheek, 's', 'c')
        swap(g, 19, cheek, 's', 'c')
    # 입 — 원본대로 두 칸. 네 칸으로 넓혀 봤더니 벌린 입이 되고, 양끝을 한 줄
    # 올려 봤더니 32칸에서는 칸 사이가 벌어져 점 세 개로 읽혔다. 대신
    # 아랫입술에 빛 한 줄을 둬서 입술이 도톰해 보이게 한다
    hfill(g, cheek + 2, 15, 16, 'm')
    hfill(g, cheek + 3, 15, 16, 'l')     # 아랫입술 빛 한 줄
...
def build_boy():
    ...
    for y in (23, 24, 25):
        put(g, 9, y, 'T')
    hfill(g, 27, 9, 10, 'U')       # 걷어올린 소맷단 — 밝은 색이라야 보인다
    hfill(g, 27, 21, 22, 'U')
    # 손 (벙어리장갑처럼 도톰하게)
    for y in range(28, 32):
        hfill(g, y, 8, 10, 's')
