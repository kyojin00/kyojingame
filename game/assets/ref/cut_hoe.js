// 호미 아이콘(skanghal) 교체 — 원본(641x641, 투명 배경)의 여백을 잘라
// 32x32 격자(기존 icon_hoe.png 규격)에 꽉 차게 앉힌다.
// 도구 아이콘·가방이 이 한 장을 같이 쓴다.
// 다시 갈면 이 스크립트만 돌리면 된다. 원본: ref/src/ref_hoe.png
// 실행: repo 루트에서  NODE_PATH=... node game/assets/ref/cut_hoe.js
const fs = require('fs'), { PNG } = require('pngjs');
const im = PNG.sync.read(fs.readFileSync(__dirname + '/src/ref_hoe.png'));
...
fs.writeFileSync(__dirname + '/../sprites/icon_hoe.png', PNG.sync.write(out));
console.log('icon_hoe.png 32x32 완료 (bbox', bw, 'x', bh, ')');
