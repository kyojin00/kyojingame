...
PAL = {
    ...
    'G2': (46, 30, 32), 'G': (76, 48, 44), 'g': (110, 72, 56),
    'h': (146, 100, 70), 'j': (184, 138, 96), 'J': (218, 180, 136),

    # ── 눈 — 그림에서 **유일하게 채도가 높은 자리**
    'e': (44, 30, 42), 'e1': (92, 68, 82),
    'i': (72, 42, 38), 'i1': (120, 72, 48), 'i2': (170, 116, 66),
    'i3': (216, 166, 98),
...
# **옆선이 어디서부터 꺾이느냐**다.
FACE_TOP = 22
FACE_F = [11.0, 12.5, 13.6, 14.4, 15.0, 15.4, 15.7, 15.9, 16.1, 16.2,
          16.3, 16.4, 16.4, 16.5, 16.5, 16.5, 16.5, 16.5, 16.4, 16.4,
          16.3, 16.2, 16.0, 15.9, 15.7, 15.4, 15.1, 14.7, 14.2, 13.6,
          12.9, 12.1, 11.2, 10.2, 9.1, 8.0, 6.8, 5.5, 4.1, 2.6]  # y 22~61
FACE_M = [11.6, 13.2, 14.4, 15.3, 16.0, 16.5, 16.9, 17.2, 17.4, 17.6,
          17.7, 17.8, 17.8, 17.8, 17.8, 17.8, 17.8, 17.8, 17.8, 17.7,
          17.7, 17.6, 17.6, 17.5, 17.4, 17.2, 17.0, 16.8, 16.5, 16.2,
          15.4, 14.8, 14.0, 13.2, 12.3, 11.3, 10.2, 9.0, 7.6, 6.0,
          4.4, 2.8]                                              # y 22~63

# ------------------------------------------------------------------ 머리 실루엣
HAIR_TOP = 12
CAP_F = [4.0, 7.6, 10.4, 12.7, 14.6, 16.1, 17.4, 18.4, 19.2, 19.9,
         20.4, 20.8, 21.1, 21.3, 21.5, 21.6, 21.7, 21.8, 21.8, 21.8,
         21.8, 21.8, 21.8, 21.8, 21.8, 21.8, 21.8, 21.8, 21.8, 21.8,
         21.8, 21.8, 21.8, 21.8, 21.8, 21.7, 21.6, 21.5, 21.3, 21.1,
         20.8, 20.5, 20.1, 19.6, 19.0]                           # y 12~56
# 턱 옆으로 흘러내리는 단발 — 바깥 반폭과 안쪽 반폭(비우는 폭)을 함께 적는다
HANG_TOP = 57
HANG_OUT = [18.7, 18.6, 18.5, 18.4, 18.2, 18.0, 17.8, 17.5, 17.2, 16.9,
            16.5, 16.1, 15.6, 15.0, 14.2, 13.2]                  # y 57~72
HANG_IN = [8.6, 8.0, 7.4, 7.0, 7.4, 7.9, 8.3, 8.6, 8.9, 9.1,
           9.3, 9.4, 9.5, 9.6, 9.7, 9.9]

CAP_M = [4.2, 8.0, 10.9, 13.2, 15.1, 16.7, 18.0, 19.0, 19.8, 20.4,
         20.9, 21.2, 21.4, 21.5, 21.5, 21.5, 21.4, 21.2, 21.0, 20.8,
         20.6, 20.4, 20.2, 20.0, 19.8, 19.6, 19.4, 19.2, 18.9, 18.5,
         18.0, 17.3, 16.4, 15.2, 13.6]                           # y 12~46
...
def head(c, sex, blink=False):
    ...
    c.blob(CX - 4, 41, 14, 21, 'H1', over={'s'})           # 밝은 면은 왼쪽 위
    c.blob(CX - 6, 32, 7, 5, 'H2', over={'H1'})            # 이마 빛
    c.blob(CX + 14, 42, 4, 16, 'S1', over={'s', 'H1'})     # 오른쪽 가장자리
    c.band(CX + 11, 42, 1, 16, 'S1', {'H1', 's'}, phase=0)  # 경계 한 줄만
    c.blob(CX + 16, 43, 2, 11, 'S2', over={'S1'})
    ...
    ey = 40 if girl else 41   # 얼굴 y22~61(여)·22~63(남) 의 절반 언저리
    lx, rx = CX - 4 - EW, CX + 4
    if blink:
