...
function cave() {
  ...
  // 기슭 — 밑동에 흙이 튀고 풀이 비집는다. 이 한 줌이 언덕을 땅에 심는다
  for (const [x, y] of [[3, 46], [5, 47], [26, 47], [29, 46], [14, 47]])
    g.px(x, y, [150, 116, 82]);
  for (const [x, y] of [[2, 45], [30, 45]]) {
    g.px(x, y, MOSS[0]); g.px(x, y - 1, MOSS[1]);
  }
  g.outline(LINE);
  return g;
}
...
function stall() {
  ...
  // 기둥 발치 — 모래가 튄 자국. 발이 모래에 묻혀야 세운 것이 된다
  for (const px3 of [4, 42]) {
    g.px(px3 - 1, 38, [190, 168, 128]); g.px(px3 + 2, 38, [172, 150, 112]);
  }
...
function lookout() {
  ...
  // 이끼와 풀, 흙 튄 발치 — 다리가 땅을 딛는 자리
  g.px(8, 46, MOSS[0]); g.px(23, 45, MOSS[1]); g.px(24, 46, MOSS[0]);
  for (const [x, y] of [[6, 46], [11, 46], [21, 46], [26, 46]])
    g.px(x, y, [150, 116, 82]);
  g.outline(LINE);
  return g;
}
